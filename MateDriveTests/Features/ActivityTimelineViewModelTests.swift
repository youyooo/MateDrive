...
final class ActivityTimelineViewModelTests: XCTestCase {
    ...
    func testIdenticalBackgroundReloadDoesNotRepublishTimelineRows() async {
        let session = makeSession(id: "stable", startDate: date(1_000), kinds: [.drive])
        let store = TimelineSessionStoreSpy(listResponses: [[session], [session]])
        let viewModel = ActivityTimelineViewModel(
            sessionStore: store,
            notificationCenter: NotificationCenter()
        )
        ...
        XCTAssertEqual(viewModel.state.sessions, [session])
    ...
    func testCarSwitchClearsOldSessionsAndRejectsInFlightOldCarResult() async {
        ...
        let newReadStarted = expectation(description: "new car refresh started")
        let newReadFulfiller = ExpectationFulfiller(newReadStarted)
        let store = TimelineSessionStoreSpy(
            ...
            listCallObserver: { call in
                ...
                if call == 3 { newReadFulfiller.fulfill() }
            }
        )
        let viewModel = ActivityTimelineViewModel(
            sessionStore: store,
        ...
        await fulfillment(of: [newReadStarted], timeout: 1)
    ...
    func testDetailPublishesSelectedCachedSessionBeforeDatabaseRefresh() async {
        ...
        let store = TimelineSessionStoreSpy(detailSession: refreshed)
        let viewModel = ActivitySessionDetailViewModel(
            sessionStore: store,
            initialSession: cached
        )

        XCTAssertEqual(viewModel.session, cached)
        XCTAssertNil(viewModel.errorMessage)

        await viewModel.load(carId: 7, sessionId: cached.id)

        XCTAssertEqual(viewModel.session, refreshed)
        XCTAssertNil(viewModel.errorMessage)
    }
