...
final class DetailLayoutTests: XCTestCase {
    func testMetricCardDecorativeIconDoesNotOverlapTitleAtAccessibilityTextSize() throws {
        let content = MetricCard(
            title: "当前容量",
            value: "",
        ...
        let iconBounds = try XCTUnwrap(pixelBounds(in: image) { red, green, blue, alpha in
            alpha > 0
                && green > 120
                && Int(green) > Int(red) + 40
                && Int(green) > Int(blue) + 20
        })
        let titleBounds = try XCTUnwrap(
            pixelBounds(in: image, xRange: 24..<320) { red, green, blue, alpha in
                alpha > 0 && red < 80 && green < 80 && blue < 80
            }
        )
        ...
        XCTAssertLessThanOrEqual(
            iconBounds.maxX + 4,
            titleBounds.minX,
            "The scaled decorative icon must keep its own layout space instead of covering the title."
        )
    }

    func testHistoryHistogramBarRemainsCompactInsideWideSingleBucket() {
        let view = HistoryHistogramBar(
            color: .green,
            height: 120
        )
        ...
        XCTAssertLessThanOrEqual(size.width, 44)
    ...
    func testDashboardTyrePressureDoesNotExpandBeyondSmallPhoneWidth() {
        ...
        let view = DashboardTyrePressureView(
            tpms: tpms,
            units: .metric,
            palette: MateDrivePalettes.light
        )
        .environment(\.appLanguage, .chinese)
    ...
    private func pixelBounds(
        in image: UIImage,
        xRange requestedXRange: Range<Int>? = nil,
        matching predicate: (_ red: UInt8, _ green: UInt8, _ blue: UInt8, _ alpha: UInt8) -> Bool
    ) -> CGRect? {
        ...
        var maxX = -1
        var maxY = -1
        let xRange = requestedXRange ?? 0..<width
        for y in 0..<height {
