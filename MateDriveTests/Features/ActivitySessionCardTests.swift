...
final class ActivitySessionCardTests: XCTestCase {
    ...
    func testAccessibilityDynamicTypeProducesFiniteExpandedHostedLayout() {
        ...
        for (label, size) in [("normal", normalSize), ("accessibility", accessibilitySize)] {
            ...
            XCTAssertLessThanOrEqual(
                size.width,
                320,
                "The \(label) hosted card must not exceed the constrained phone width: \(size)"
            )
        }
    ...
    private func makePresentation(
        ...
        ActivitySessionCardBuilder.presentation(
            session: SmartActivitySession(
                ...
                latitude: SyntheticCoordinates.point(latitudeOffset: 0.2).northing,
                longitude: SyntheticCoordinates.point(longitudeOffset: 0.9).easting,
                geofenceID: "home",
