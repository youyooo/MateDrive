...
public struct EnvironmentHistoryView: View {
    ...
    public var body: some View {
        Group {
            if viewModel.state.isLoading, viewModel.state.response == nil {
                ...
                LoadingStateView(
                    title: t("Environment history unavailable", "环境历史不可用"),
                    message: error,
        ...
        .accessibilityIdentifier("environment_history_view")
    ...
    private func summary(_ response: EnvironmentHistoryResponse) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                summaryMetric(t("Lowest", "最低胎压"), response.data.summary?.lowestPressure?.value.map { MateDriveUnitFormatter.formatPressure(pressureBar($0, response: response), units: units) } ?? "--", "tirepressure")
                summaryMetric(t("Avg Outside", "平均车外"), response.data.summary?.averageOutsideTemp.map { MateDriveUnitFormatter.formatTemperature(temperatureCelsius($0, response: response), units: units, decimals: 1) } ?? "--", "thermometer.medium")
            }
        }
    }
    ...
    private func temperatureChart(_ response: EnvironmentHistoryResponse) -> some View {
        let units = resolvedUnits(response)
        return chartSection(title: t("Temperature", "温度"), subtitle: MateDriveUnitFormatter.temperatureUnit(units: units)) {
            Chart {
                ForEach(viewModel.datedPoints) { point in
                    if let date = point.timestamp, let value = point.outsideTemp {
                        LineMark(x: .value("Time", date), y: .value("Temperature", MateDriveUnitFormatter.temperatureValue(temperatureCelsius(value, response: response), units: units)), series: .value("Series", t("Outside", "车外")))
                            .foregroundStyle(by: .value("Series", t("Outside", "车外"))).interpolationMethod(.catmullRom)
                        PointMark(x: .value("Time", date), y: .value("Temperature", MateDriveUnitFormatter.temperatureValue(temperatureCelsius(value, response: response), units: units)))
                            .foregroundStyle(.blue).symbolSize(18)
                    }
                    if let date = point.timestamp, let value = point.insideTemp {
                        LineMark(x: .value("Time", date), y: .value("Temperature", MateDriveUnitFormatter.temperatureValue(temperatureCelsius(value, response: response), units: units)), series: .value("Series", t("Inside", "车内")))
    ...
    private func pressureChart(_ response: EnvironmentHistoryResponse) -> some View {
        let units = resolvedUnits(response)
        let lines = tireLines
        return chartSection(title: t("Tire Pressure", "胎压"), subtitle: MateDriveUnitFormatter.pressureUnit(units: units)) {
            Chart {
                ForEach(lines) { line in
                    ForEach(viewModel.datedPoints) { point in
                        if let date = point.timestamp, let pressure = line.value(point) {
                            LineMark(x: .value("Time", date), y: .value("Pressure", MateDriveUnitFormatter.pressureValue(pressureBar(pressure, response: response), units: units)), series: .value("Wheel", line.title))
                                .foregroundStyle(by: .value("Wheel", line.title))
                                .interpolationMethod(.catmullRom)
                            PointMark(x: .value("Time", date), y: .value("Pressure", MateDriveUnitFormatter.pressureValue(pressureBar(pressure, response: response), units: units)))
                                .foregroundStyle(by: .value("Wheel", line.title))
                                .symbolSize(18)
                        }
                    }
    ...
    private func energyChart(_ response: EnvironmentHistoryResponse) -> some View {
        if !response.data.temperatureEnergyBuckets.isEmpty {
            let units = resolvedUnits(response)
            chartSection(title: t("Temperature and Consumption", "温度与电耗"), subtitle: MateDriveUnitFormatter.efficiencyUnit(units: units)) {
                Chart(response.data.temperatureEnergyBuckets) { bucket in
                    if let from = bucket.temperatureFrom, let to = bucket.temperatureTo, let value = bucket.medianConsumptionWhPerUnit {
                        BarMark(
                            x: .value("Temperature", MateDriveUnitFormatter.temperatureValue(temperatureCelsius((from + to) / 2, response: response), units: units)),
                            y: .value("Consumption", MateDriveUnitFormatter.efficiencyValue(consumptionWhPerKm(value, response: response), units: units))
                        )
    ...
    private func temperatureSelection(_ point: EnvironmentHistoryPoint, response: EnvironmentHistoryResponse) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 2) {
            if let value = point.outsideTemp { Text(t("Outside", "车外") + " " + MateDriveUnitFormatter.formatTemperature(temperatureCelsius(value, response: response), units: units, decimals: 1)) }
            if let value = point.insideTemp { Text(t("Inside", "车内") + " " + MateDriveUnitFormatter.formatTemperature(temperatureCelsius(value, response: response), units: units, decimals: 1)) }
        }
    ...
    private func pressureSelection(_ point: EnvironmentHistoryPoint, response: EnvironmentHistoryResponse) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 2) {
            ForEach(tireLines) { line in
                if let value = line.value(point) { Text(line.title + " " + MateDriveUnitFormatter.formatPressure(pressureBar(value, response: response), units: units)) }
            }
        }
        .font(.caption2.monospacedDigit()).padding(5).background(.regularMaterial, in: RoundedRectangle(cornerRadius: 6))
    }
    ...
    private func signedPressure(_ value: Double, response: EnvironmentHistoryResponse) -> String { let units = resolvedUnits(response); let converted = MateDriveUnitFormatter.pressureValue(pressureBar(value, response: response), units: units); return String(format: "%+.2f %@", converted, MateDriveUnitFormatter.pressureUnit(units: units)) }
