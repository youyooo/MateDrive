...
public enum BatteryHealthConfidence: Equatable, Sendable {
    ...
    public func detail(recordingStartOdometerKm: Double?, units: UnitPreferences?, language: AppLanguage) -> String {
        let odometer = recordingStartOdometerKm.map { MateDroidUnitFormatter.formatDistance($0, units: units, decimals: 0) }
        switch self {
        case .calibratedLateHistory:
            if let odometer {
                return localized(
...
public final class BatteryViewModel: ObservableObject {
    ...
    public init(api: any AnalyticsAPIProviding, ratedEfficiencyFallback: Double? = nil, settingsStore: (any SettingsStoring)? = nil, initialState: BatteryState = BatteryState()) {
        self.api = api
        ...
        case let (.success(health), .success(payload)):
            ...
            if let settingsStore {
                ...
                if calibration.recordingStartOdometerKm != recordingStartOdometer {
                    ...
                    settings.setBatteryCalibration(calibration, for: carId)
    ...
    public static func computeStats(
        ...
        if let batteryReferenceCapacityKWh, batteryReferenceCapacityKWh > 0,
           let currentCapacity = health.currentCapacity, currentCapacity > 0 {
            ...
            healthSource = startsLate ? .recordedPeriod : .teslaMateHealth
            ...
            healthSource = startsLate ? .recordedPeriod : .rangeRatio
        } else {
            healthPercent = nil
        ...
        if (healthSource == .manualCapacityReference || healthSource == .manualReference) && (batteryRecordingStartOdometerKm ?? 0) >= 50_000 {
    ...
    private static func earliestValidOdometer(_ values: Double?...) -> Double? {
        values.compactMap { $0 }
            .filter { $0.isFinite && $0 > 0 }
            .min()
    }
