...
final class TeslaMateConnectionDiagnosticTests: XCTestCase {
    func testDiagnosticRejectsRemotePlainHTTPWithActionableMessage() async {
        let report = await TeslaMateConnectionDiagnostic(client: RouteHTTPClient(routes: [:])).run(
            settings: AppSettings(serverURL: "http://review.example:3030"),
        ...
        XCTAssertEqual(report.checks.first?.title, "服务器安全")
        XCTAssertTrue(report.checks.first?.message.contains("必须使用 HTTPS") == true)
    }

    func testDiagnosticExplainsWhenTailscaleHostIsUnreachable() async {
        let report = await TeslaMateConnectionDiagnostic(client: UnreachableHTTPClient()).run(
            settings: AppSettings(serverURL: "https://youyooomac-mini.tail8e46b1.ts.net"),
            authenticator: RequestAuthenticator(),
            language: .chinese
        )

        let vehicleCheck = report.checks.first(where: { $0.checkID == .cars })
        XCTAssertEqual(vehicleCheck?.status, .failed)
        XCTAssertTrue(vehicleCheck?.message.contains("Tailscale") == true)
        XCTAssertTrue(vehicleCheck?.message.contains("局域网") == true)
    }
    ...
    func testDiagnosticExportRedactsCredentialsHostsAndCoordinates() {
        ...
        let report = TeslaMateDiagnosticReport(checks: [
            TeslaMateDiagnosticCheck(
                ...
                message: "GET \(credentialedEndpoint)?token=example-token failed; Authorization: Bearer example-bearer; password=correct horse battery staple; location 12.345678, 98.765432"
            )
        ])
        ...
        for secret in ["teslamate.example", "192.0.2.10", "example-user", "example-password", "token=example-token", "Bearer example-bearer", "correct horse battery staple", "horse battery staple", "12.345678", "98.765432"] {
    ...
    func testDiagnosticExportStrictlyRedactsPersonalAndLocalIdentifiers() {
        let privateValues = [
            "owner@example.com",
            "username=youyooo",
            "teslamate.local:3030",
            "localhost:3030",
            "[fe80::1234]:3030",
            "fe80::1234",
            "LRW3E7FA1NC123456",
            "8F14E45F-EA7A-4C35-9A91-1F2D3E4C5B6A",
            "/Users/youyooo/Documents/private.json",
            "28.228,112.939"
        ]
        let report = TeslaMateDiagnosticReport(checks: [
            TeslaMateDiagnosticCheck(
                checkID: .configuration,
                status: .failed,
                title: "Local account owner@example.com",
                message: privateValues.joined(separator: "; ")
            )
        ])
        ...
        for value in privateValues {
            XCTAssertFalse(text.contains(value), "Export leaked: \(value)")
        }
        XCTAssertTrue(text.contains("format_version: 2"))
        XCTAssertTrue(text.contains("redaction_profile: strict-v2"))
        XCTAssertTrue(text.contains("[REDACTED_EMAIL]"))
        XCTAssertTrue(text.contains("[REDACTED_PATH]"))
        XCTAssertTrue(text.contains("[REDACTED_IDENTIFIER]"))
    }
    ...
    func testDiagnosticReportsReadableCoreEndpointsAndDataWarnings() async {
        ...
        XCTAssertEqual(report.checks.first(where: { $0.checkID == .vehicleInfo })?.message, "可以识别车型 Model 3，但外观颜色或轮毂类型缺失。")
    ...
    func testDiagnosticWarnsWhenVehicleInfoCannotIdentifyDashboardModel() async {
        ...
        XCTAssertEqual(vehicleInfo?.message, "可以读取车辆列表，但车型和车辆名称缺失，车辆识别信息不完整。")
    }
    ...
    func testDiagnosticFallsBackToSecondaryServerWhenPrimaryStatusIsRetryableFailure() async {
        let diagnostic = TeslaMateConnectionDiagnostic(client: RouteHTTPClient(routes: [
            "primary.example/api/v1/cars": (200, #"{"data":{"cars":[{"car_id":1,"display_name":"Blue 3","car_details":{"model":"3"}}]}}"#),
            "primary.example/api/v1/cars/1/status": (500, #"{"error":"offline"}"#),
            "backup.example/api/v1/cars": (200, #"{"data":{"cars":[{"car_id":1,"display_name":"Blue 3","car_details":{"model":"3"}}]}}"#),
            "backup.example/api/v1/cars/1/status": (200, #"{"data":{"status":{"battery_details":{"battery_level":50}},"units":{"unit_of_length":"km"}}}"#),
            "backup.example/api/v1/globalsettings": (200, #"{"data":{"settings":{"unit_of_length":"km","unit_of_temperature":"C","preferred_range":"rated"}}}"#),
            "backup.example/api/v1/cars/1/drives": (200, #"{"data":{"drives":[]}}"#),
        ...
        XCTAssertEqual(report.checks.first?.message, "正在使用 https://backup.example")
        XCTAssertEqual(report.checks.first(where: { $0.checkID == .cars })?.message, "找到 1 辆车。")
        XCTAssertEqual(report.checks.first(where: { $0.checkID == .status })?.status, .passed)
    }
...
private struct RouteHTTPClient: HTTPClient, @unchecked Sendable {
    ...
    func data(for request: URLRequest) async throws -> (Data, HTTPURLResponse) {
        ...
        let page = request.url
            ...
            .flatMap { $0.first(where: { $0.name == "page" })?.value }
            .flatMap(Int.init) ?? 1
        let route: (statusCode: Int, json: String)
        if page > 1, path.hasSuffix("/drives") {
            route = (200, #"{"data":{"drives":[]}}"#)
        } else if page > 1, path.hasSuffix("/charges") {
            route = (200, #"{"data":{"charges":[]}}"#)
        } else {
            route = routes[hostPath] ?? routes[path] ?? (404, #"{"error":"not found"}"#)
        }
...
private struct UnreachableHTTPClient: HTTPClient, @unchecked Sendable {
    func data(for request: URLRequest) async throws -> (Data, HTTPURLResponse) {
