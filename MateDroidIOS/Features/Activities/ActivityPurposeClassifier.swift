...
public enum ActivityPurposeClassifier {
    ...
    private static func geofenceClassification(_ input: ActivityClassificationInput) -> ActivityClassificationResult? {
        ...
        switch geofenceKind {
        case .home:
            classification = (input.hasCharge ? .homeCharging : .parking, 0.85, [.homeGeofence])
        case .work:
            classification = (input.hasCharge ? .workCharging : .parking, 0.85, [.workGeofence])
        case .charging:
            classification = (input.hasCharge ? .replenishment : .parking, input.hasCharge ? 0.90 : 0.85, [.chargingGeofence])
        case .shopping:
            classification = (.shopping, 0.85, [.shoppingGeofence])
        case .schoolPickup:
