...
public enum TeslaMateDiagnosticExport {
    ...
    private static func safe(_ value: String) -> String {
        ...
        let redactions: [(String, String)] = [
            ...
            (#"(?<!\d)-?\d{1,3}\.\d{3,}\s*[,/]\s*-?\d{1,3}\.\d{3,}(?!\d)"#, "[REDACTED_COORDINATES]"),
            (#"(?i)\bBearer\s+[A-Za-z0-9._~+/=-]+"#, "[REDACTED_CREDENTIAL]"),
            (#"(?i)\b(?:authorization|api[_ -]?token|token|access[_ -]?key|secret(?:[_ -]?key)?|password|username|user)\b\s*[:=]\s*(?:"[^"]*"|'[^']*'|[^,;|]+)"#, "[REDACTED_CREDENTIAL]"),
            (#"(?i)(?:/Users|/home|/var/mobile)/[^\s,;]+"#, "[REDACTED_PATH]"),
            (#"(?i)\[[0-9a-f:]+\](?::\d+)?"#, "[REDACTED_HOST]"),
            (#"(?i)\b[0-9a-f]{1,4}::[0-9a-f:]*\b"#, "[REDACTED_HOST]"),
            (#"(?i)\b(?:localhost|[a-z0-9-]+\.local)(?::\d+)?\b"#, "[REDACTED_HOST]"),
            (#"\b(?:\d{1,3}\.){3}\d{1,3}(?::\d+)?\b"#, "[REDACTED_HOST]"),
            (#"\b(?:[A-Za-z0-9-]+\.)+[A-Za-z]{2,}(?::\d+)?\b"#, "[REDACTED_HOST]"),
            (#"(?i)\b[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\b"#, "[REDACTED_IDENTIFIER]"),
            (#"\b[A-HJ-NPR-Z0-9]{17}\b"#, "[REDACTED_IDENTIFIER]"),
