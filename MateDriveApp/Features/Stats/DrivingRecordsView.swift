...
public struct DrivingRecordsView: View {
    ...
    public var body: some View {
        Group {
            if viewModel.state.isLoading, viewModel.state.records.isEmpty {
                ...
                LoadingStateView(
                    title: t("Driving records unavailable", "驾驶纪录不可用"),
                    message: error,
                    systemImage: "trophy",
        ...
        .accessibilityIdentifier("driving_records_view")
    ...
    private func value(_ record: StatsExtreme) -> String {
        ...
        switch record.type {
        case "longest_distance", "daily_most_distance": return MateDriveUnitFormatter.formatDistance(value, units: units, decimals: 1)
        case "longest_duration": return MateDriveUnitFormatter.formatDuration(minutes: Int(value.rounded()), language: appLanguage)
        case "highest_speed": return MateDriveUnitFormatter.formatSpeed(value, units: units, decimals: 0)
        case "highest_elevation_gain": return MateDriveUnitFormatter.formatElevation(Int(value.rounded()), units: units)
        case "hottest_day", "coldest_day": return MateDriveUnitFormatter.formatTemperature(value, units: units, decimals: 1)
