...
private struct ParkingActivityDetailView: View {
    @Environment(\.appLanguage) private var appLanguage
    @Environment(\.appDisplayUnitSystem) private var appDisplayUnitSystem
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 145), spacing: 12)], spacing: 12) {
                    ...
                    MetricCard(title: t("Odometer", "里程表"), value: activity.odometerKm.map { MateDroidUnitFormatter.formatDistance($0, units: units, decimals: 0) } ?? "--", systemImage: "car")
                }
            }
            .padding(16)
        }
    ...
    private func signedDistance(_ value: Double) -> String {
        let converted = MateDroidUnitFormatter.distanceValue(value, units: units)
        return String(format: "%+.1f %@", converted, MateDroidUnitFormatter.distanceUnit(units: units))
    }
