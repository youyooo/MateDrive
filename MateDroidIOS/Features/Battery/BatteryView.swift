...
public struct BatteryView: View {
    ...
    public var body: some View {
        Group {
            if viewModel.state.isLoading {
                ...
                LoadingStateView(title: t("Battery unavailable", "电池数据不可用"), message: viewModel.state.errorMessage, systemImage: "battery.0percent")
            }
        }
    ...
    private func gauge(_ stats: BatteryStats) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if !stats.showsAbsoluteHealth {
                Label(t("Calibration required", "需要校准"), systemImage: "exclamationmark.triangle.fill")
                    .font(.title2.bold())
    ...
    private func metrics(_ stats: BatteryStats) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 145), spacing: 12)], spacing: 12) {
                ...
                MetricCard(title: batteryTitle("Efficiency"), value: stats.ratedEfficiency.map { MateDroidUnitFormatter.formatEfficiency($0, units: viewModel.state.units.resolved(for: appDisplayUnitSystem), decimals: 0) } ?? "--", systemImage: "leaf")
                MetricCard(title: batteryTitle("Health Source"), value: stats.healthSource.title(language: appLanguage), systemImage: "info.circle")
                MetricCard(title: batteryTitle("Health Confidence"), value: stats.confidence.title(language: appLanguage), systemImage: "checkmark.seal")
            ...
            Text(stats.confidence.detail(recordingStartOdometerKm: stats.recordingStartOdometerKm, units: viewModel.state.units.resolved(for: appDisplayUnitSystem), language: appLanguage))
                .font(.footnote)
                .foregroundStyle(stats.confidence == .lateHistoryNeedsReference ? .orange : .secondary)
        }
    }
           ...
           let summary = viewModel.state.historySummary {
            VStack(alignment: .leading, spacing: 12) {
                ...
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 145), spacing: 12)], spacing: 12) {
                    ...
                    MetricCard(title: t("Derived Efficiency", "样本推导效率"), value: summary.efficiencyWhKm.map { MateDroidUnitFormatter.formatEfficiency($0, units: viewModel.state.units.resolved(for: appDisplayUnitSystem), decimals: 0) } ?? "--", systemImage: "leaf")
    ...
    private func distanceText(_ value: Double) -> String {
        MateDroidUnitFormatter.formatDistance(value, units: viewModel.state.units.resolved(for: appDisplayUnitSystem))
    }
