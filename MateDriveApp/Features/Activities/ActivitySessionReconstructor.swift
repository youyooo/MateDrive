    ...
    private static func classified(
        ...
        let classification = ActivityPurposeClassifier.classify(
            ActivityClassificationInput(
                ...
                arrivalMinute: arrivalDate.map { date in
                    let parts = calendar.dateComponents([.hour, .minute], from: date)
                    return (parts.hour ?? 0) * 60 + (parts.minute ?? 0)
                },
                isWeekday: arrivalDate.map { date in
                    let weekday = calendar.component(.weekday, from: date)
                    return (2 ... 6).contains(weekday)
                },
                dwellMinutes: session.endDate.map {
                    max(0, Int($0.timeIntervalSince(session.startDate) / 60))
                }
            )
        )
