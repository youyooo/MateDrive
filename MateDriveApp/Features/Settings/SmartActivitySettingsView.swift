...
public final class SmartActivitySettingsModel: ObservableObject {
    ...
    private let carIds: @MainActor () -> [Int]
    ...
    public func confirmPendingAction() async {
        ...
        do {
            switch action {
            case .rebuildDerivedActivities:
                let resolvedCarIds = try await knownCarIds()
                guard !resolvedCarIds.isEmpty else {
                    lastResult = .failed("No vehicle is available for rebuilding.")
                    return
                }
                let report = await indexer.forceRebuild(carIds: resolvedCarIds)
                lastResult = .rebuilt(
            ...
            case .clearLearnedSuggestions:
                let resolvedCarIds = try await knownCarIds()
                for carId in resolvedCarIds {
                    let overrides = try await labelStore.overrides(carId: carId)
                    for value in overrides where value.scope == .futureAtPlace {
                        try await labelStore.delete(id: value.id)
                    }
                }
                try await pricingObservationStore.removeAll()
                _ = await indexer.forceRebuild(carIds: resolvedCarIds)
    ...
    private func knownCarIds() async throws -> [Int] {
        let indexedCarIds = try await indexer.knownCarIds()
        let labeledCarIds = try await labelStore.knownCarIds()
        return Array(Set(carIds() + indexedCarIds + labeledCarIds)).sorted()
    }
}
...
public struct SmartActivitySettingsView<GeofenceDestination: View, ChargePricingDestination: View>: View {
    ...
    public var body: some View {
        Form {
            Section(t("Rules", "规则")) {
                NavigationLink {
                    ActivityLabelRulesView(labelStore: labelStore, indexer: indexer, carIds: carIds)
                } label: {
                    Label(t("Activity Labels", "动态标签"), systemImage: "tag.fill")
                }
                ...
                if let status = model.catalogStatus {
                    ...
                    Label(
                        UserFacingErrorLocalizer.localized(
                            error,
                            language: settingsViewModel.settings.appLanguage
                        ),
...
@MainActor
private struct ActivityLabelRulesView: View {
    @Environment(\.appLanguage) private var appLanguage
    @State private var labels: [ActivityLabelOverride] = []
    @State private var errorMessage: String?

    let labelStore: any ActivityLabelOverrideStoring
    let indexer: any SmartActivityIndexing
    let carIds: @MainActor () -> [Int]
    ...
    var body: some View {
        List {
            ...
            ForEach(labels) { label in
                VStack(alignment: .leading, spacing: 5) {
                    Label(labelTitle(label), systemImage: label.icon)
                        .font(.body.weight(.semibold))
                    Text(scopeTitle(label.scope))
                        .font(.caption)
        ...
        .overlay {
            if labels.isEmpty, errorMessage == nil {
                ContentUnavailableView(
                    t("No saved activity labels", "暂无已保存的动态标签"),
                    systemImage: "tag"
                )
            }
        }
        .navigationTitle(t("Activity Labels", "动态标签"))
        .task { await load() }
    }

    private func load() async {
        do {
            var loaded: [ActivityLabelOverride] = []
            let knownCarIds = try await labelStore.knownCarIds()
            for carId in Set(carIds() + knownCarIds) {
                loaded.append(contentsOf: try await labelStore.overrides(carId: carId))
            }
    ...
    private func delete(_ label: ActivityLabelOverride) async {
        do {
            try await labelStore.delete(id: label.id)
            labels.removeAll { $0.id == label.id }
            _ = await indexer.forceRebuild(carIds: [label.carId])
        } catch {
            errorMessage = error.localizedDescription
        }
    }

    private func labelTitle(_ label: ActivityLabelOverride) -> String {
        let customName = label.customName?.trimmingCharacters(in: .whitespacesAndNewlines)
        if let customName, !customName.isEmpty {
            return customName
        }
        return label.purpose.title(language: appLanguage)
    }

    private func scopeTitle(_ scope: ActivityLabelScope) -> String {
        switch scope {
        case .sessionOnly: return t("This activity only", "仅本次动态")
        case .futureAtPlace: return t("Future activities at this place", "此地点以后的动态")
        }
    }
