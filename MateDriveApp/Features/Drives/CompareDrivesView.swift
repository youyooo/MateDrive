...
public struct CompareDrivesView: View {
    ...
    public var body: some View {
        ...
        .accessibilityIdentifier("compare_drives_view")
        .task {
    ...
    private var average: some View {
        ...
        return LazyVGrid(columns: [GridItem(.adaptive(minimum: 145), spacing: 12)], spacing: 12) {
            ...
            MetricCard(title: t("Avg Efficiency", "平均效率"), value: viewModel.state.average.efficiency.map { MateDriveUnitFormatter.formatEfficiency($0, units: units, decimals: 0) } ?? "--", systemImage: "leaf")
        }
    }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        HStack(spacing: 10) {
                            ...
                            if let efficiency = row.efficiency {
                                pill(MateDriveUnitFormatter.formatEfficiency(efficiency, units: units, decimals: 0))
                            }
                        }
                    }
                    .padding(14)
