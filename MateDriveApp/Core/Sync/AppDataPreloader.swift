...
public actor AppDataPreloader {
    ...
    public func preload(force: Bool = false) async -> AppDataPreloadReport {
        ...
        return AppDataPreloadReport(
            didRun: true,
            requestedEndpointCount: requests.count + widgetResult.requestedRequestCount + activitiesResult.requestedPageCount
    ...
    private func loadDetailSnapshots(
        ...
        return await withTaskGroup(of: (requested: Int, successful: Int).self) { group in
            for car in cars {
                group.addTask {
                    async let localHistory = Self.localHistory(
                        carId: car.carId,
                    ...
                    let (history, drives, charges) = await (localHistory, drivesResult, chargesResult)
                    var requested = 2
                    var successful = [drives.isSuccess, charges.isSuccess].filter { $0 }.count
                    var detailRequests: [DetailRequest] = []

                    let recentDriveIDs: [Int]
                    if case let .success(values) = drives {
                        recentDriveIDs = values.sorted { lhs, rhs in
                    ...
                    let driveIDs = Self.uniqueIDs(
                        recentDriveIDs + history.drives.map(\.driveId),
                        limit: 300
                    )
                    detailRequests += driveIDs.enumerated().map { index, id in
                        DetailRequest(
                    ...
                    let recentChargeIDs: [Int]
                    if case let .success(values) = charges {
                        recentChargeIDs = values.sorted { lhs, rhs in
                            let lhsDate = lhs.startDate.flatMap(DomainDateParser.date(from:)) ?? .distantPast
                            let rhsDate = rhs.startDate.flatMap(DomainDateParser.date(from:)) ?? .distantPast
                            return lhsDate > rhsDate
                        }.compactMap(\.chargeId)
                    ...
                    let chargeIDs = Self.uniqueIDs(
                        recentChargeIDs + history.charges.map(\.chargeId),
                        limit: 1_000
                    )
                    detailRequests += chargeIDs.enumerated().map { index, id in
                        DetailRequest(
            ...
            for car in cars {
                group.addTask {
                    ...
                    while page <= 100, !Task.isCancelled {
                        ...
                        requested += 1
                        let path = "api/v1/cars/\(car.carId)/places"
                        let probeResult: APIResult<TeslaMateProbeResponse>
                        if page == 1 {
                            probeResult = await api.probe(
                                path: path,
                                queryItems: queryItems,
                                cachePolicy: .reloadIgnoringLocalCacheData
                            )
                        } else {
                            let cached = await api.probe(
                                path: path,
                                queryItems: queryItems,
                                cachePolicy: .returnCacheDataDontLoad
                            )
                        ...
                        guard case let .success(probe) = probeResult else { break }
                        successful += 1
    ...
    private func loadWidgetSnapshots(
        ...
        let results = await withTaskGroup(
            of: (snapshot: WidgetVehicleSnapshot?, requested: Int, successful: Int).self
        ) { group in
            for car in cars {
                group.addTask {
                    ...
                    async let chargesResult = api.refreshCharges(carId: car.carId, page: 1, show: 200)
                    async let localHistory = Self.localHistory(
                        carId: car.carId,
                        databaseProvider: databaseProvider
                    )
                    let resolvedStatusResult = await statusResult
                    let currentChargeResult: APIResult<CurrentChargeOutcome>?
                    if resolvedStatusResult.value?.status?.isCharging == true {
                        currentChargeResult = await api.refreshCurrentCharge(carId: car.carId)
                    } else {
                        currentChargeResult = nil
                    }
                    let (resolvedHealthResult, resolvedHistoryResult, resolvedChargesResult, resolvedLocalHistory) = await (
                        healthResult,
                        historyResult,
                        chargesResult,
                        localHistory
                    )
                    let previous = previousByID[identifier]
                    ...
                    let data = payload.map {
                        Self.widgetDisplayData(car: car, payload: $0, settings: settings)
                    } ?? previous?.data ?? Self.widgetDisplayData(
                        car: car,
                        payload: nil,
                        settings: settings
                    )
                    let batteryTrend: WidgetBatteryTrendData?
                    if resolvedHealthResult.isSuccess, resolvedHistoryResult.isSuccess {
                        batteryTrend = await Self.batteryTrendData(
                            carId: car.carId,
                            healthResult: resolvedHealthResult,
                            historyResult: resolvedHistoryResult,
                        ...
                        batteryTrend = previous?.batteryTrend
                    }
                    let chargingTrend: WidgetChargingTrendData?
                    if resolvedChargesResult.isSuccess {
                        chargingTrend = Self.chargingTrendData(
                            chargesResult: Self.mergedCharges(
                                remote: resolvedChargesResult,
                                local: resolvedLocalHistory.charges
                            ),
                            settings: settings,
                            referenceDate: referenceDate
                        )
                    } else {
                        chargingTrend = previous?.chargingTrend
                    }
                    let currentCharge = WidgetCurrentChargeSnapshotBuilder.build(
                        statusResult: resolvedStatusResult,
                        currentChargeResult: currentChargeResult,
                        previous: previous?.currentCharge,
                        now: referenceDate
                    )
                    ...
                    let successfulRequestCount = [
                        resolvedStatusResult.isSuccess,
                        resolvedHealthResult.isSuccess,
                        resolvedHistoryResult.isSuccess,
                        resolvedChargesResult.isSuccess,
                        currentChargeResult?.isSuccess == true
                    ].filter { $0 }.count
                    let requestedRequestCount = 4 + (currentChargeResult == nil ? 0 : 1)
                    return (
            ...
            var requestedRequestCount = 0
            var successfulRequestCount = 0
            for await (snapshot, requested, successful) in group {
                if let snapshot { snapshots.append(snapshot) }
                requestedRequestCount += requested
                successfulRequestCount += successful
            }
            return WidgetLoadResult(
                requestedRequestCount: requestedRequestCount,
        ...
        let currentByID = Dictionary(uniqueKeysWithValues: results.snapshots.map { ($0.id, $0) })
        let identifiers = Set(previousByID.keys).union(currentByID.keys)
        if identifiers.contains(where: { previousByID[$0]?.data != currentByID[$0]?.data }) {
            await widgetTimelineReloader.reloadTimelines(ofKind: WidgetConstants.carStatusKind)
        }
        if identifiers.contains(where: { previousByID[$0]?.batteryTrend != currentByID[$0]?.batteryTrend }) {
            await widgetTimelineReloader.reloadTimelines(ofKind: WidgetConstants.batteryTrendKind)
        }
        if identifiers.contains(where: { previousByID[$0]?.chargingTrend != currentByID[$0]?.chargingTrend }) {
            await widgetTimelineReloader.reloadTimelines(ofKind: WidgetConstants.chargingTrendKind)
        }
        if identifiers.contains(where: { previousByID[$0]?.currentCharge != currentByID[$0]?.currentCharge }) {
            await widgetTimelineReloader.reloadTimelines(ofKind: WidgetConstants.currentChargeKind)
        }
        return results
    }

    private static func localHistory(
        carId: Int,
        databaseProvider: any AppDatabaseProviding
    ) async -> (drives: [DriveSummaryRecord], charges: [ChargeSummaryRecord]) {
        do {
            ...
            return try await (drives, charges)
        } catch {
            return ([], [])
        }
    }

    private static func uniqueIDs(_ values: [Int], limit: Int) -> [Int] {
        var seen: Set<Int> = []
        return values.filter { seen.insert($0).inserted }.prefix(limit).map { $0 }
    }

    private static func mergedCharges(
        remote: APIResult<[ChargeData]>,
        local: [ChargeSummaryRecord]
    ) -> APIResult<[ChargeData]> {
        let remoteValues = remote.value ?? []
        let localValues = local.map {
            ChargeData(
                chargeId: $0.chargeId,
                carId: $0.carId,
                ...
                endDate: $0.endDate,
                address: $0.address,
                chargeEnergyAdded: $0.chargeEnergyAdded,
                chargeEnergyUsed: $0.chargeEnergyUsed,
                cost: $0.cost,
                durationMin: $0.durationMin,
                odometer: $0.odometerKm,
                latitude: $0.latitude,
                longitude: $0.longitude,
                startBatteryLevel: $0.startBatteryLevel,
                endBatteryLevel: $0.endBatteryLevel
            )
        }
        var seen: Set<Int> = []
        let merged = (remoteValues + localValues).filter { charge in
            guard let id = charge.chargeId else { return false }
            return seen.insert(id).inserted
        }
        if !merged.isEmpty || remote.isSuccess {
    ...
    private static func requests(carId: Int, referenceDate: Date) -> [Request] {
        ...
        let firstPage = [
            ...
            URLQueryItem(name: "show", value: "200")
        ]
        let formatter = ISO8601DateFormatter()
