...
public enum DashboardTextFormatter {
    ...
    public static func updatesText(_ updates: Int?, language: AppLanguage) -> String? {
        ...
        case .german:
            return updates == 1 ? "1 Softwareupdate" : "\(updates) Softwareupdates"
        case .spanish:
            return updates == 1 ? "1 actualización de software" : "\(updates) actualizaciones de software"
        case .italian:
            return updates == 1 ? "1 aggiornamento software" : "\(updates) aggiornamenti software"
        case .catalan:
            return updates == 1 ? "1 actualització de programari" : "\(updates) actualitzacions de programari"
        case .english, .system:
    ...
    private static func dynamicCountLabels(language: AppLanguage) -> (charge: String, drive: String) {
        ...
        case .traditionalChinese:
            return ("次充電", "次行程")
        case .german:
            return ("Ladevorgänge", "Fahrten")
        case .spanish:
            return ("cargas", "trayectos")
        case .italian:
            return ("ricariche", "viaggi")
        case .catalan:
            return ("carregues", "trajectes")
        case .english, .system:
    ...
    private static func resolvedDynamicLanguage(_ language: AppLanguage) -> AppLanguage {
        ...
        if preferred.hasPrefix("ca") { return .catalan }
        return .english
    }
