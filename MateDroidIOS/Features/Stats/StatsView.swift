...
public struct StatsView: View {
                    ...
                    LazyHGrid(rows: Array(repeating: GridItem(.fixed(12), spacing: 3), count: 7), spacing: 3) {
                        ForEach(Array(heatmapCells.enumerated()), id: \.offset) { _, activity in
                            RoundedRectangle(cornerRadius: 2, style: .continuous)
                                ...
                                .accessibilityLabel(activity.map(accessibilityText) ?? "")
                                .accessibilityHidden(activity == nil)
                        }
                    }
                    .frame(height: 102)
                }
    ...
    private var records: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let efficient = viewModel.state.summary.mostEfficientDrive {
                recordButton(title: localizedTitle("Most Efficient"), value: efficient.efficiencyWhKm.map { MateDroidUnitFormatter.formatEfficiency($0, units: viewModel.state.units.resolved(for: appDisplayUnitSystem), decimals: 0) } ?? "--", drive: efficient)
            }
        }
    }
    ...
    private func distanceText(_ value: Double) -> String {
        MateDroidUnitFormatter.formatDistance(value, units: resolvedUnits)
    }
    ...
    private func efficiencyText(_ value: Double?) -> String {
        value.map { MateDroidUnitFormatter.formatEfficiency($0, units: viewModel.state.units.resolved(for: appDisplayUnitSystem), decimals: 0) } ?? noDataText
    }
    ...
    private func accessibilityText(_ activity: StatsDailyActivity) -> String {
        let date = activity.date.formatted(date: .abbreviated, time: .omitted)
        switch heatmapMetric {
        case .distance:
            return "\(date), \(distanceText(activity.distanceKm)), \(activity.driveCount) " + t("drives", "次行程")
        case .chargingEnergy:
            return "\(date), \(String(format: "%.1f kWh", activity.chargeEnergyKWh)), \(activity.chargeCount) " + t("charges", "次充电")
        }
    }
...
public enum StatsUnitPresentation {
    ...
    public static func drainRate(_ kilometersPerHour: Double, units: UnitPreferences?) -> String {
        let value = MateDroidUnitFormatter.distanceValue(kilometersPerHour, units: units)
        return String(format: "%.2f %@/h", value, MateDroidUnitFormatter.distanceUnit(units: units))
    }
}
