...
private struct DriveMetricDetailContent: View {
    ...
    private var rows: [DriveMetricDetailRow] {
        ...
        case .averageSpeed:
            return [
                ...
                row("Summary fallback", "汇总备用字段", detail.speedAvg.map { MateDroidUnitFormatter.formatSpeed($0, units: units) } ?? "--")
            ]
        case .maxSpeed:
            return [
                ...
                row("Position samples", "位置采样", "\(speeds.count)"),
                row("Summary speedMax", "汇总最高速度", detail.speedMax.map { MateDroidUnitFormatter.formatSpeed(Double($0), units: units) } ?? "--")
            ]
        case .efficiency:
            return [
    ...
    private func valueText(for metric: DriveMetricKind) -> String {
        ...
        case .efficiency:
            return stats.efficiency.map { MateDroidUnitFormatter.formatEfficiency($0, units: units, decimals: 0) } ?? "--"
        case .energy:
    ...
    private func formatElevation(_ value: Int?) -> String {
        value.map { MateDroidUnitFormatter.formatElevation($0, units: units) } ?? "--"
    }
