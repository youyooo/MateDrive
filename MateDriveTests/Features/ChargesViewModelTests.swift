...
final class ChargesViewModelTests: XCTestCase {
    func testAPIChargeSummaryProviderReturnsPersistentCacheWhenServerIsUnavailable() async throws {
        let fixtureLatitude = 12.3456
        let fixtureLongitude = 34.5678
        let cached = ChargeSummaryItem(
            chargeId: 20,
            carId: 1,
        ...
        let cache = InMemoryChargeSummaryCache(items: [cached])
        let provider = APIChargeSummaryProvider(api: FailingChargeSummaryAPI(), cache: cache)

        let items: [ChargeSummaryItem]
        switch await provider.chargeSummaries(carId: 1) {
        case let .success(value):
            items = value
        case let .failure(error):
            XCTFail("Expected cached charges, got \(error)")
            return
        }
    ...
    func testViewModelShowsPersistentCacheBeforeFullHistoryRequestCompletes() async {
        let gate = ChargeSummaryGate()
        let store = CacheFirstChargeSummaryStore(gate: gate)
        ...
        XCTAssertEqual(viewModel.state.rows.map(\.chargeId), [41])

        await gate.release()
        await loadTask.value

        XCTAssertEqual(viewModel.state.rows.map(\.chargeId), [42])
    }

    func testRepeatedChargeListLoadKeepsCachedContentAndStartsOneRefresh() async {
        let gate = ChargeSummaryGate()
        let viewModel = ChargesViewModel(
            store: CacheFirstChargeSummaryStore(gate: gate),
            showShortEntries: true
        )

        let firstLoad = Task { await viewModel.load(carId: 1) }
        await gate.waitUntilStarted()
        for _ in 0..<20 where viewModel.state.rows.isEmpty {
            await Task.yield()
        }

        let repeatedLoad = Task { await viewModel.load(carId: 1) }
    ...
    func testCostFilterTreatsAPIZeroAsMissingAndManualZeroAsExplicitFree() async throws {
        let store = InMemoryChargeSummaryStore(items: [
        ...
        let overrides = InMemoryChargeCostOverrideStore(overrides: [2: 0])
        let viewModel = ChargesViewModel(
            store: store,
            showShortEntries: true,
    ...
    func testChargeCostPresentationMakesEstimateAndCorrectionStatusExplicit() {
        XCTAssertEqual(ChargeCostPresentation.status(.pricingRule, language: .chinese), "估算")
        XCTAssertEqual(ChargeCostPresentation.status(.api, language: .chinese), "已记录")
        XCTAssertEqual(ChargeCostPresentation.status(.manual, language: .chinese), "已确认")
        XCTAssertEqual(ChargeCostPresentation.status(.none, language: .chinese), "待补充")
        XCTAssertEqual(ChargeCostPresentation.status(.pricingRule, language: .english), "Estimated")
        XCTAssertEqual(
    ...
    func testPositiveTeslaMateCostWinsAutomaticRuleEstimate() async throws {
        let store = InMemoryChargeSummaryStore(items: [
            ChargeSummaryItem(
                chargeId: 1,
                carId: 1,
        ...
        let viewModel = ChargesViewModel(store: store, settingsStore: settingsStore)

        await viewModel.load(carId: 1)

        XCTAssertEqual(viewModel.state.rows.first?.cost, 3)
        XCTAssertEqual(viewModel.state.rows.first?.costSource, .api)
        XCTAssertEqual(viewModel.state.summary.apiCostCount, 1)
        XCTAssertEqual(viewModel.state.summary.pricingRuleCostCount, 0)
    }

    func testChargeListUsesSameProbableHomeRegionalEstimateAsDashboard() async throws {
        let store = InMemoryChargeSummaryStore(items: [
            ChargeSummaryItem(
                chargeId: 9,
                carId: 1,
        ...
        let viewModel = ChargesViewModel(
            store: store,
            settingsStore: StaticChargesSettingsStore(settings: AppSettings(currencyCode: "CNY"))
        )

        await viewModel.load(carId: 1)

        XCTAssertEqual(viewModel.state.rows.first?.cost ?? -1, 9.0972, accuracy: 0.001)
        XCTAssertEqual(viewModel.state.rows.first?.costSource, .pricingRule)
        XCTAssertTrue(viewModel.state.rows.first?.isHistoricalReference == true)
        XCTAssertEqual(viewModel.state.summary.pricingRuleCostCount, 1)
    }
    ...
    func testPricingRulesScaleBatterySamplesToWallEnergyForBilling() async throws {
        let store = InMemoryChargeSummaryStore(items: [
            ChargeSummaryItem(
                chargeId: 1,
                carId: 1,
        ...
        XCTAssertEqual(viewModel.state.rows.first?.cost ?? 0, 24, accuracy: 0.001)
        XCTAssertEqual(viewModel.state.summary.totalCost, 24, accuracy: 0.001)
    }
...
private struct CacheFirstChargeSummaryStore: ChargeSummaryProviding {
    let gate: ChargeSummaryGate

    func cachedChargeSummaries(carId _: Int) async -> [ChargeSummaryItem] {
        [ChargeSummaryItem(chargeId: 41, carId: 1, startDate: "2026-07-01T09:00:00Z", chargeEnergyAdded: 8)]
    }

    func chargeSummaries(carId _: Int) async -> APIResult<[ChargeSummaryItem]> {
        await gate.waitForRelease()
        return .success([
            ChargeSummaryItem(chargeId: 42, carId: 1, startDate: "2026-07-02T09:00:00Z", chargeEnergyAdded: 9)
        ])
    }
}

private actor ChargeSummaryGate {
...
private struct FailingChargeSummaryAPI: ChargeAPIProviding {
    ...
    func currentCharge(carId _: Int) async -> APIResult<CurrentChargeOutcome> { .failure(.network("offline")) }
    func chargeDetail(carId _: Int, chargeId _: Int) async -> APIResult<ChargeDetail> { .failure(.network("offline")) }
    func carStatus(carId _: Int) async -> APIResult<CarStatusPayload> { .failure(.network("offline")) }
}
