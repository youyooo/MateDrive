...
public struct FeatureHubView: View {
    ...
    private func featureContent(presentation: FeatureHubPresentation) -> some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: 24, pinnedViews: []) {
                vehicleHeader(presentation: presentation)

                ForEach(presentation.sections) { section in
                    VStack(alignment: .leading, spacing: 10) {
                        Text(verbatim: section.title(language: appLanguage))
                            .font(.headline)
    ...
    @ViewBuilder
    private func vehicleHeader(presentation: FeatureHubPresentation) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(verbatim: presentation.vehicleName)
                .font(.title3.weight(.semibold))
                .lineLimit(2)
                .minimumScaleFactor(0.82)

            if presentation.showsVehiclePicker,
               let selectedCarId = viewModel.state.selectedCarId {
                Picker(
                    t("Vehicle", "车辆"),
                    selection: Binding(
                        get: { selectedCarId },
                    ...
                    ForEach(viewModel.state.cars) { car in
                        Text(verbatim: car.name).tag(car.id)
                    }
                }
    ...
    private func featureButton(_ item: FeatureHubItem) -> some View {
        Button {
            ...
            VStack(alignment: .leading, spacing: 9) {
                ...
                Text(verbatim: item.title(language: appLanguage))
                    .font(.subheadline.weight(.semibold))
                    .foregroundStyle(.primary)
                    .lineLimit(2)
                    .fixedSize(horizontal: false, vertical: true)

                Text(verbatim: item.subtitle(language: appLanguage))
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    .lineLimit(2)
                    .fixedSize(horizontal: false, vertical: true)
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(Text(verbatim: item.title(language: appLanguage)))
        .accessibilityHint(Text(verbatim: item.subtitle(language: appLanguage)))
    }
