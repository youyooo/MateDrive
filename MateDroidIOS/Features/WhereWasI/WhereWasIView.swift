...
public struct WhereWasIView: View {
    ...
    public var body: some View {
        Group {
            if viewModel.state.isLoading {
                ...
                LoadingStateView(title: t("Unable to find location", "无法查询位置"), message: UserFacingErrorLocalizer.localized(error, language: appLanguage), systemImage: "exclamationmark.triangle")
            } else {
                ScrollView {
    ...
    private var map: some View {
        if let location = GeoCoordinateValidator.location(latitude: viewModel.state.latitude, longitude: viewModel.state.longitude) {
            ...
            Map(initialPosition: .region(MKCoordinateRegion(center: coordinate, span: MKCoordinateSpan(latitudeDelta: 0.03, longitudeDelta: 0.03)))) {
    ...
    private var metrics: some View {
        ...
        return LazyVGrid(columns: [GridItem(.adaptive(minimum: 145), spacing: 12)], spacing: 12) {
            if let distance = viewModel.state.driveDistance {
                MetricCard(title: t("Drive Distance", "行驶距离"), value: MateDroidUnitFormatter.formatDistance(distance, units: units), systemImage: "road.lanes")
            }
            if let speed = viewModel.state.speed {
                MetricCard(title: t("Speed", "速度"), value: MateDroidUnitFormatter.formatSpeed(Double(speed), units: units), systemImage: "speedometer")
            }
            ...
            if let temp = viewModel.state.outsideTemp {
                MetricCard(title: t("Outside", "车外"), value: MateDroidUnitFormatter.formatTemperature(temp, units: units), systemImage: "thermometer.medium")
            }
