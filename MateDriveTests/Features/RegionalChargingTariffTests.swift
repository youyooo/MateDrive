...
final class RegionalChargingTariffTests: XCTestCase {
    func testRegionCanBeInferredFromLocalizedAddress() throws {
        let catalog = try RegionalChargingTariffCatalog.load()

        XCTAssertEqual(catalog.regionCode(matchingAddress: "长沙市望城区, 湖南省, 中国"), "CN-43")
        XCTAssertEqual(catalog.regionCode(matchingAddress: "Hunan, China"), "CN-43")
        XCTAssertEqual(catalog.regionCode(matchingAddress: "长科路, 岳麓区"), "CN-43")
        XCTAssertNil(catalog.regionCode(matchingAddress: "Unknown place"))
    }
    ...
    func testExpiredTariffEstimateIsExplicitlyMarkedAndLocalizedAsHistoricalReference() throws {
        let catalog = try RegionalChargingTariffCatalog.load()
        let currentDate = try XCTUnwrap(DomainDateParser.date(from: "2026-07-18T12:00:00+08:00"))

        let rule = try XCTUnwrap(catalog.estimationRule(regionCode: "CN-43", date: currentDate))

        XCTAssertTrue(rule.isHistoricalReference)
        ...
        XCTAssertEqual(
            ...
            "Hunan residential EV historical reference"
        )
    }
