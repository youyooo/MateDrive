...
public struct ChargeCostExplanation: Equatable, Sendable {
    public let status: String
    public let summary: String
    public let evidence: [String]
    public let allowsCorrection: Bool

    public init(
        status: String,
        summary: String,
        evidence: [String],
        allowsCorrection: Bool
    ) {
        self.status = status
...
public enum ChargePricingMatchFactor: String, Equatable, Hashable, Sendable {
    case savedLocation
    case inferredHome
    case regionalTariff
...
public enum ChargeCostExplanationBuilder {
    public static func build(
        source: ChargeCostSource,
        ruleName: String?,
        matchFactors: [ChargePricingMatchFactor],
        components: [ChargePricingCostComponent],
        sessionFee: Double,
        language: AppLanguage
    ) -> ChargeCostExplanation? {
        switch source {
        case .none:
            return nil
        case .manual:
            return ChargeCostExplanation(
                status: localized("Confirmed", "已确认", language: language),
                summary: localized(
                    "This amount was entered by you and takes priority over every automatic estimate.",
                    "此金额由你录入，并始终优先于所有自动估算。",
                    language: language
                ),
                evidence: [],
        ...
        case .api:
            return ChargeCostExplanation(
                status: localized("Recorded", "已记录", language: language),
                summary: localized(
                    "This amount comes from the TeslaMate charge record.",
                    "此金额来自 TeslaMate 充电记录。",
                    language: language
                ),
                evidence: [],
                allowsCorrection: true
        ...
        case .pricingRule:
            var evidence: [String] = []
            let usesHistoricalReference = matchFactors.contains(.historicalReference)
            if let ruleName = cleaned(ruleName) {
                evidence.append(localized("Matched rule: \(ruleName)", "匹配规则：\(ruleName)", language: language))
            }
            evidence.append(contentsOf: unique(matchFactors).map {
                matchFactorText($0, language: language)
            })
            let timeBasedComponents = components.filter {
                $0.startMinuteOfDay != nil && $0.endMinuteOfDay != nil
            }
            if !timeBasedComponents.isEmpty {
                evidence.append(localized(
                    "Calculated from \(timeBasedComponents.count) time-based rate segment\(timeBasedComponents.count == 1 ? "" : "s").",
                    "按 \(timeBasedComponents.count) 个分时电价区间计算。",
                    language: language
                ))
            }
            let energy = components.compactMap(\.energyKWh).filter { $0.isFinite && $0 >= 0 }.reduce(0, +)
            if energy > 0 {
                evidence.append(localized(
                    String(format: "Pricing energy: %.2f kWh", energy),
                    String(format: "计价电量：%.2f kWh", energy),
                    language: language
                ))
            }
            if sessionFee.isFinite, sessionFee > 0 {
                evidence.append(localized("Includes a session fee.", "包含单次服务费。", language: language))
            }
            return ChargeCostExplanation(
                status: usesHistoricalReference
                    ? localized("Historical reference", "历史参考", language: language)
                    : localized("Estimated", "估算", language: language),
                summary: usesHistoricalReference
                    ? localized(
                        "MateDrive estimated this amount from an expired official tariff. It is a reference only until you confirm or correct it.",
                        "MateDrive 使用已到期的官方电价估算此金额；在你确认或修正前，它仅供参考。",
                        language: language
                    )
                    : localized(
                        "MateDrive estimated this amount from the matched pricing rule and available charging data.",
                        "MateDrive 根据匹配的价格规则和可用充电数据估算此金额。",
                        language: language
                    ),
                evidence: evidence,
                allowsCorrection: true
    ...
    private static func matchFactorText(
        _ factor: ChargePricingMatchFactor,
        language: AppLanguage
    ) -> String {
        switch factor {
        case .savedLocation:
            return localized("Matched the saved station location", "匹配已保存的充电地点", language: language)
        case .inferredHome:
            return localized(
                "Inferred probable home charging from the overnight AC pattern; no Home geofence was matched",
                "根据夜间交流充电模式推测可能为家充；未匹配到家庭围栏",
                language: language
            )
        case .regionalTariff:
            return localized("Matched the selected regional tariff", "匹配已选择的地区电价", language: language)
        case .historicalReference:
            return localized(
                "Uses an expired official tariff as a historical reference; verify the amount before relying on it",
                "使用已到期的官方电价作为历史参考，请核对后再采用该金额",
                language: language
            )
        case .chargingTime:
            return localized("Matched the charging time window", "匹配充电时段", language: language)
        case .chargerType:
            return localized("Matched the AC or DC charger type", "匹配交流或直流充电类型", language: language)
        case .weekday:
            return localized("Matched the day-of-week rule", "匹配星期规则", language: language)
        case .seasonalDate:
            return localized("Matched the tariff effective date", "匹配电价生效日期", language: language)
        }
    }

    private static func unique(
        _ values: [ChargePricingMatchFactor]
    ) -> [ChargePricingMatchFactor] {
        var seen = Set<ChargePricingMatchFactor>()
...
public struct ChargePricingRule: Codable, Equatable, Identifiable, Sendable {
    ...
    public var isHistoricalReference: Bool {
        id.hasSuffix("-reference-estimate")
    }
...
public enum ChargePricingRuleEngine {
    static let maximumInterpolatedInterval: TimeInterval = 7 * 24 * 60 * 60
    ...
    public static func estimateCost(for input: ChargePricingInput, rules: [ChargePricingRule]) -> ChargePricingEstimate? {
        ...
        guard !hasUnsafeInterpolationGap(for: rule, input: input, totalEnergy: energy) else {
            return nil
        }
    ...
    private static func hasUnsafeInterpolationGap(
        for rule: ChargePricingRule,
        input: ChargePricingInput,
        totalEnergy: Double
    ) -> Bool {
        guard !rule.timeSegments.isEmpty, totalEnergy > 0 else { return false }
        ...
        if let sessionStart, let sessionEnd, sessionEnd < sessionStart {
            return true
        }
        ...
        if samples.isEmpty {
            ...
            return sessionEnd.timeIntervalSince(sessionStart) > maximumInterpolatedInterval
        }
        ...
        for sample in samples {
            ...
            let acceptedEnergy = min(
                sample.1 - previousEnergy,
                max(0, totalEnergy - allocatedEnergy)
            )
            if acceptedEnergy > 0,
            ...
            if allocatedEnergy >= totalEnergy { return false }
        }

        guard totalEnergy > allocatedEnergy else { return false }
        let residualEnd = sessionEnd ?? previousDate
        let residualDuration = residualEnd.timeIntervalSince(previousDate)
        return residualDuration < 0 || residualDuration > maximumInterpolatedInterval
    }
