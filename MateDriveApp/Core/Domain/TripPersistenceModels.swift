...
public enum TripTimelineBuilder {
    ...
    public static func build(trip: DetectedTrip, dcChargeIds: Set<Int>, showShort: Bool) -> [TripTimelineSegment] {
        ...
        // Timeline views preserve every source record. Filtering source data can
        // make totals and detail screens disagree, so the legacy preference is
        // accepted for decoding but no longer hides drives or charges.
        let drives = trip.drives
        let charges = trip.charges
...
public extension DetectedTrip {
    ...
    func displayName(language: AppLanguage) -> String {
        ...
        let usesChinese = MateDriveUnitFormatter.usesChineseLabels(language: language)
