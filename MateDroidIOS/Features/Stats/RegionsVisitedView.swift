    ...
    private func row(_ region: RegionVisitRecord) -> some View {
        HStack {
            ...
            VStack(alignment: .trailing, spacing: 4) {
                Text(GeographyCoveragePresentation.valuePrefix(missingCount: region.missingDriveDistanceCount) + MateDroidUnitFormatter.formatDistance(region.totalDistanceKm, units: viewModel.state.units.resolved(for: appDisplayUnitSystem)))
