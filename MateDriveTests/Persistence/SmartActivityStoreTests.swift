...
final class SmartActivityStoreTests: XCTestCase {
    ...
    func testKnownCarIdsIncludeIndexedEmptyCarsAndLabeledCars() async throws {
        let stores = try await SmartActivityTestStores.make()
        try await stores.sessions.replace(
            carId: 4,
            sessions: [],
            derivationFingerprint: "empty-car"
        )
        try await stores.sessions.replace(
            carId: 2,
            sessions: [SmartActivitySession.fixture(id: "car-2", carId: 2)]
        )
        try await stores.labels.save(ActivityLabelOverride.fixture(id: "car-7-label", carId: 7))
        let indexedCarIds = try await stores.sessions.knownCarIds()
        let labeledCarIds = try await stores.labels.knownCarIds()

        XCTAssertEqual(indexedCarIds, [2, 4])
        XCTAssertEqual(labeledCarIds, [7])
    }
