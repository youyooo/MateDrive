...
public struct DatabaseBackupProvider: DatabaseBackupProviding, Sendable {
    ...
    public func createArtifact(appVersion: String) async throws -> DatabaseBackupArtifact {
        ...
        do {
            ...
            var backupSettings = await settingsStore.load()
            backupSettings.serverURL = Self.backupSafeURL(backupSettings.serverURL)
            backupSettings.secondaryServerURL = Self.backupSafeURL(backupSettings.secondaryServerURL)
            backupSettings.teslamateBaseURL = Self.backupSafeURL(backupSettings.teslamateBaseURL)
            let settingsData = try encoder.encode(backupSettings)
    ...
    private static func backupSafeURL(_ value: String) -> String {
