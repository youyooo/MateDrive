...
public struct ChargesView: View {
                    ...
                    ForEach(viewModel.state.chartData) { point in
                        VStack(spacing: 6) {
                            ...
                            RoundedRectangle(cornerRadius: 4, style: .continuous)
                                .fill(.tint)
                                .frame(height: max(12, CGFloat((point.totalEnergy ?? 0) / maxEnergy) * 120))
    ...
    private func rowSubtitle(_ row: ChargeRow) -> String {
        let cost = row.cost.map(formatCost) ?? "--"
        return "\(dateText(row.startDate)) · \(ChargesPresentation.energyText(row.energyAdded, isComplete: row.energyAdded != nil)) · \(cost) · \(row.costSource.displayText(language: appLanguage))"
    }
