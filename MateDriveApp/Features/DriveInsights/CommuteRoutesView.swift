...
public struct CommuteRoutesView: View {
    ...
    public var body: some View {
        Group {
            if viewModel.state.isLoading, viewModel.state.routes.isEmpty {
                ...
                LoadingStateView(
                    title: t("Commute routes unavailable", "通勤路线不可用"),
                    message: error,
        ...
        .accessibilityIdentifier("commute_routes_view")
    ...
    private func distance(_ value: Double?) -> String { value.map { MateDriveUnitFormatter.formatDistance($0, units: resolvedUnits, decimals: 1) } ?? "--" }
...
private struct CommuteRouteDetailView: View {
    ...
    private var elevation: String {
        ...
        return "+\(MateDriveUnitFormatter.formatElevation(Int(gain.rounded()), units: units)) / −\(MateDriveUnitFormatter.formatElevation(Int(loss.rounded()), units: units))"
    }
    ...
    private func distance(_ value: Double?) -> String { value.map { MateDriveUnitFormatter.formatDistance($0, units: units, decimals: 1) } ?? "--" }
    private func duration(_ value: Double?) -> String { value.map { MateDriveUnitFormatter.formatDuration(minutes: Int($0.rounded()), language: appLanguage) } ?? "--" }
