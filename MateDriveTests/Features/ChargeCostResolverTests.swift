...
final class ChargeCostResolverTests: XCTestCase {
    func testBillingEvidenceUsesWallEnergyAndScalesBatterySamples() throws {
        let result = ChargeBillingEvidence.energyAndSamples(
            wallEnergy: 12,
            batteryEnergy: 10,
            samples: [
                ChargePricingEnergySample(date: "2026-07-01T18:00:00+08:00", cumulativeEnergyAddedKWh: 0),
                ChargePricingEnergySample(date: "2026-07-01T19:00:00+08:00", cumulativeEnergyAddedKWh: 10)
            ]
        )

        XCTAssertEqual(result.energy, 12)
        XCTAssertEqual(try XCTUnwrap(result.samples.last?.cumulativeEnergyAddedKWh), 12, accuracy: 0.001)
        XCTAssertEqual(ChargeBillingEvidence.recordedCost(8), 8)
        XCTAssertNil(ChargeBillingEvidence.recordedCost(0))
        XCTAssertNil(ChargeBillingEvidence.recordedCost(.nan))
    }

    func testOvernightInferenceUsesChargeTimestampTimeZoneAndKeeps22KWAC() throws {
        let record = ChargeSummaryRecord(
            chargeId: 91,
            carId: 1,
            startDate: "2026-07-19T23:30:00-07:00",
            endDate: "2026-07-20T00:30:00-07:00",
        ...
        let result = AutomaticChargeCostEstimator.resolve(
            record: record,
            settings: AppSettings(currencyCode: "CNY"),
            manualCost: nil,
            catalog: try RegionalChargingTariffCatalog.load()
        )

        XCTAssertEqual(result.resolution?.source, .regionalTariff)
        XCTAssertNotNil(result.resolution)
        XCTAssertTrue(result.resolution?.isHistoricalReference == true)
        XCTAssertEqual(result.locationInference.source, .overnightACPattern)
        XCTAssertEqual(result.locationInference.kind, .home)
        XCTAssertLessThan(result.locationInference.confidence, 0.8)
    }

    func testAutomaticHomeEstimateIgnoresAPIZeroAndUsesFenceTimeAndWallEnergy() throws {
        ...
        let record = ChargeSummaryRecord(
            chargeId: 7,
            carId: 1,
            startDate: "2026-07-19T00:00:00+08:00",
        ...
        XCTAssertEqual(result.displayAddress, "家")
        XCTAssertEqual(result.resolution?.amount ?? -1, 5.04, accuracy: 0.001)
        XCTAssertEqual(result.resolution?.source, .regionalTariff)
        XCTAssertTrue(result.resolution?.isEstimated == true)
        XCTAssertEqual(result.locationInference.source, .savedGeofence)
        XCTAssertEqual(result.locationInference.kind, .home)
        XCTAssertEqual(result.locationInference.confidence, 1)
    }

    func testAutomaticHomeInferenceNeverOverridesSavedNonHomeGeofence() throws {
        let coordinate = SyntheticCoordinates.point()
        let settings = AppSettings(
            currencyCode: "CNY",
            geofenceRules: [
        ...
        let record = ChargeSummaryRecord(
            ...
            endDate: "2026-07-20T03:00:00+08:00",
            chargeEnergyAdded: 16,
            cost: 0,
            durationMin: 240,
            address: "长沙市, 湖南省, 中国",
            latitude: coordinate.northing,
        ...
        XCTAssertEqual(result.locationInference.source, .savedGeofence)
        XCTAssertEqual(result.locationInference.kind, .shopping)
        XCTAssertEqual(result.locationInference.confidence, 1)
        XCTAssertNil(result.resolution)
    }

    func testDaytimeACWithoutSavedFenceIsNotInferredAsHome() throws {
        let record = ChargeSummaryRecord(
            chargeId: 11,
            carId: 1,
            startDate: "2026-07-19T12:00:00+08:00",
            endDate: "2026-07-19T16:00:00+08:00",
            chargeEnergyAdded: 16,
            cost: 0,
            durationMin: 240,
            address: "长沙市, 湖南省, 中国",
        ...
        XCTAssertEqual(result.locationInference.source, .none)
        XCTAssertNil(result.locationInference.kind)
        XCTAssertEqual(result.locationInference.confidence, 0)
        XCTAssertNil(result.resolution)
    }

    func testAutomaticHomeEstimateKeepsExplicitManualFreeOverride() throws {
        let record = ChargeSummaryRecord(
            chargeId: 8,
            carId: 1,
            startDate: "2026-07-19T00:00:00+08:00",
        ...
        XCTAssertEqual(result.resolution?.amount, 0)
        XCTAssertEqual(result.resolution?.source, .manual)
        XCTAssertTrue(result.resolution?.isExplicitlyFree == true)
    }

    func testLongOvernightACCanUseRegionalEstimateBeforeUserCreatesHomeFence() throws {
        let record = ChargeSummaryRecord(
            chargeId: 9,
            carId: 1,
            startDate: "2026-07-19T23:00:00+08:00",
        ...
        XCTAssertEqual(result.resolution?.amount ?? -1, 9.0972, accuracy: 0.001)
        XCTAssertEqual(result.resolution?.source, .regionalTariff)
        XCTAssertTrue(result.resolution?.isEstimated == true)
        XCTAssertTrue(result.resolution?.isHistoricalReference == true)
        XCTAssertEqual(result.locationInference.source, .overnightACPattern)
        XCTAssertEqual(result.locationInference.kind, .home)
        XCTAssertEqual(result.locationInference.confidence, 0.75)
    }
    ...
    func testConfirmedStationRuleWinsRegionalHomeFallbackAndZeroAPIIsIgnored() {
        ...
        XCTAssertEqual(result?.amount ?? 0, 6.6, accuracy: 0.000_001)
    }
    ...
    func testSessionOnlyConfirmationSavesManualCostAndObservationWithoutRule() async throws {
        ...
        try await service.confirm(ChargePricingConfirmation(
            ...
            fixedFee: nil,
            currencyCode: "CNY",
            coordinates: GeocodeLocation(latitude: SyntheticCoordinates.point().northing, longitude: SyntheticCoordinates.point().easting),
            chargerIdentity: .ac,
            startMinute: nil,
            endMinute: nil
        ))
    ...
    func testFutureConfirmationDerivesUnitPriceAndCreatesScopedLearnedRule() async throws {
        ...
        let coordinates = GeocodeLocation(latitude: SyntheticCoordinates.point(latitudeOffset: 0.000_056).northing, longitude: SyntheticCoordinates.point(longitudeOffset: 0.000_089).easting)
        ...
        let stationKey = "12.0001,34.0001:teslaSupercharger"
    ...
    func testRollbackFailureIsReportedExplicitly() async {
        let observations = ResolverFailingObservationStore(failRemove: true)
        let overrides = ResolverFailingCostOverrideStore(initialValue: 4)
        let settings = ResolverFailingSettingsStore(AppSettings(), failFirstSave: true)
        let service = ChargePricingObservationService(
            observationStore: observations,
            costOverrideStore: overrides,
            settingsStore: settings
        ...
        do {
            try await service.confirm(
                futureConfirmation(chargeId: 30, stationKey: "station-rollback-failure", unitPrice: 1)
            )
            XCTFail("Expected rollback failure")
        } catch {
            XCTAssertEqual(error as? ChargePricingObservationServiceError, .rollbackFailed)
        }
    }

    func testZeroBilledEnergyIsRejectedBeforeWrites() async {
        let observations = ResolverTestObservationStore()
        let overrides = ResolverTestCostOverrideStore()
        ...
        let value = ChargePricingConfirmation(
            carId: 1,
            chargeId: 31,
            stationKey: "station-zero-energy",
            scope: .sessionOnly,
            finalAmount: 0,
            billedEnergyKWh: 0,
            pricePerKWh: nil,
        ...
        do {
            ...
            XCTFail("Expected invalid billed energy")
        } catch {
            XCTAssertEqual(error as? ChargePricingObservationServiceError, .invalidBilledEnergy)
        }
        let persistedOverride = try? await overrides.costOverride(carId: 1, chargeId: 31)
        XCTAssertNil(persistedOverride)
    }
    ...
    func testCancelledQueuedConfirmationNeverWritesPriceData() async throws {
        ...
        let firstValue = futureConfirmation(
            chargeId: 44,
            stationKey: "station-active",
            unitPrice: 1
        )
        let cancelledValue = futureConfirmation(
            chargeId: 45,
            stationKey: "station-cancelled",
            unitPrice: 2
        )

        let first = Task {
            try await service.confirm(firstValue)
        }
        await observations.waitUntilSaveStarts()

        let cancelled = Task {
            try await service.confirm(cancelledValue)
        }
        var queuedCount = await service.queuedConfirmationCount
        for _ in 0..<100 where queuedCount == 0 {
            await Task.yield()
            queuedCount = await service.queuedConfirmationCount
        }
        ...
        do {
            ...
            XCTFail("Expected queued confirmation cancellation")
        } catch {
        ...
        let cancelledObservations = try await observations.observations(
            carId: 1,
            stationKey: "station-cancelled"
        )
        let cancelledOverride = try await overrides.costOverride(carId: 1, chargeId: 45)
        let rules = await settings.load().chargePricingRules
        XCTAssertTrue(cancelledObservations.isEmpty)
        XCTAssertNil(cancelledOverride)
        XCTAssertFalse(rules.contains { $0.stationKey == "station-cancelled" })
    }

    func testCancellationDuringConfirmationRollsBackPartialWrites() async {
        let observations = ResolverSuspendingObservationStore()
        let overrides = ResolverTestCostOverrideStore()
        let settings = ResolverTestSettingsStore(AppSettings())
        let service = ChargePricingObservationService(
        ...
        let value = futureConfirmation(
            chargeId: 46,
            stationKey: "station-partial",
            unitPrice: 1
        )
        ...
        do {
            ...
            XCTFail("Expected in-progress confirmation cancellation")
        } catch {
            XCTAssertTrue(error is CancellationError)
        }

        let persistedObservations = try? await observations.observations(
            carId: 1,
            stationKey: "station-partial"
        )
        let persistedOverride = try? await overrides.costOverride(carId: 1, chargeId: 46)
        let rules = await settings.load().chargePricingRules
        XCTAssertTrue(persistedObservations?.isEmpty == true)
        XCTAssertNil(persistedOverride)
        XCTAssertFalse(rules.contains { $0.stationKey == "station-partial" })
    }
...
private actor ResolverFailingObservationStore: ChargePricingObservationStoring {
    ...
    init(failFirstSave: Bool = false, failRemove: Bool = false) {
