...
public struct TripDetailView: View {
    ...
    public var body: some View {
        ...
        .accessibilityIdentifier("trip_detail_view")
        .sheet(isPresented: $showsAddLegs) {
            addLegSheet
        }
        .sheet(isPresented: $showsMergeTrips) { mergeTripSheet }
        .sheet(isPresented: $showsShareComposer) {
            if let trip = viewModel.state.trip {
                TripShareComposerView(
                    trip: trip,
                    routeSegments: viewModel.state.routeSegments,
                    countryCount: viewModel.state.countrySummary?.countries.count ?? 0,
                    units: resolvedUnits,
    ...
    private func header(_ trip: DetectedTrip) -> some View {
        return VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .center, spacing: 10) {
                ...
                .buttonStyle(.plain)
                .accessibilityLabel(t("Share Trip Report", "分享路程报告"))
            }
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 145), spacing: 12)], spacing: 12) {
                MetricCard(title: t("Distance", "距离"), value: MateDriveUnitFormatter.formatDistance(trip.totalDistance, units: resolvedUnits, decimals: 0), systemImage: "road.lanes")
                MetricCard(title: t("Duration", "时长"), value: MateDriveUnitFormatter.formatDuration(minutes: trip.totalDurationMin, language: appLanguage), systemImage: "clock")
                MetricCard(
                    title: t("Charged", "已充电"),
                ...
                MetricCard(
                    title: t("Max Speed", "最高速度"),
                    value: trip.maxSpeed.map { MateDriveUnitFormatter.formatSpeed($0, units: resolvedUnits) } ?? "--",
        ...
        if coordinates.count >= 2 {
            VStack(alignment: .leading, spacing: 10) {
                ...
                MapGestureGate { isMapInteractionEnabled in
                    Map(initialPosition: .region(routeRegion(coordinates)), interactionModes: isMapInteractionEnabled ? .all : []) {
    ...
    private func countryRow(_ country: TripCountryStat) -> some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .trailing, spacing: 4) {
                Text(MateDriveUnitFormatter.formatDistance(country.distanceKm, units: resolvedUnits))
                    .font(.body.weight(.semibold).monospacedDigit())
                Text(TripEnergyPresentation.text(country.chargedEnergyKWh, isComplete: country.chargedEnergyIsComplete))
                    .font(.caption.monospacedDigit())
                    .foregroundStyle(.secondary)
            }
    ...
    private func localizedCountryName(code: String, fallback: String) -> String {
        let locale = Locale(identifier: MateDriveUnitFormatter.usesChineseLabels(language: appLanguage) ? "zh_Hans" : "en_US")
    ...
    private func segmentRow(_ segment: TripTimelineSegment, showsDisclosure: Bool) -> some View {
        HStack {
                    ...
                    Spacer()
                    Text(MateDriveUnitFormatter.formatDuration(minutes: segment.durationMin, language: appLanguage))
                        .font(.caption.monospacedDigit())
                        .foregroundStyle(.secondary)
                    if showsDisclosure {
                        Image(systemName: "chevron.right")
                ...
                Button {
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        Text(candidate.trip.displayName(language: appLanguage))
                            .foregroundStyle(.primary)
                        Text("\(dateText(candidate.snapshot.startDate)) · \(MateDriveUnitFormatter.formatDistance(candidate.trip.totalDistance, units: resolvedUnits, decimals: 0))")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
    ...
    private func legDetail(_ leg: TripEditableLeg) -> String {
        if let distance = leg.distanceKm {
            return MateDriveUnitFormatter.formatDistance(distance, units: resolvedUnits)
        }
