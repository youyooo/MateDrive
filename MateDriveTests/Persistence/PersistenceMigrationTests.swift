...
final class PersistenceMigrationTests: XCTestCase {
    func testMigratingEmptyDatabaseCreatesRequiredTablesAtCurrentVersionWithoutInvalidatingDetailAggregates() async throws {
        ...
        XCTAssertEqual(SchemaVersion.current, 15)
    }
    ...
    func testDriveSummaryPersistsRouteIntelligenceFields() async throws {
        let database = try SQLiteDatabase.inMemory()
        try await Migrations.applyAll(to: database)
        let store = DriveSummaryStore(database: database)
        let fingerprintJSON = #"{"points":[{"latitude":28.19,"longitude":112.98}]}"#
        let record = DriveSummaryRecord(
            driveId: 9,
            carId: 1,
            ...
            speedAvg: 34,
            outsideTempAvg: 28,
            routeFingerprintJSON: fingerprintJSON,
            climateOnFraction: 0.75,
            elevationGainM: 32,
            elevationLossM: 18
        )

        try await store.upsertAll([record])
        let records = try await store.records(carId: 1)
        let loaded = try XCTUnwrap(records.first)

        XCTAssertEqual(loaded.startAddress, "家")
        XCTAssertEqual(loaded.endAddress, "公司")
        XCTAssertEqual(loaded.routeFingerprintJSON, fingerprintJSON)
        XCTAssertEqual(loaded.climateOnFraction, 0.75)
        XCTAssertEqual(loaded.elevationGainM, 32)
        XCTAssertEqual(loaded.elevationLossM, 18)
    }
    ...
    private func assertTask3CurrentSchema(
        ...
        let requiredDriveColumns: Set<String> = [
            "energy_consumed_net", "consumption_net", "energy_source",
            "start_battery_level", "end_battery_level", "start_rated_range_km",
            "end_rated_range_km", "start_address", "end_address", "speed_avg",
            "outside_temp_avg", "route_fingerprint_json", "climate_on_fraction",
            "elevation_gain_m", "elevation_loss_m"
        ]
