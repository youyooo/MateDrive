...
public struct SoftwareUpdatesState: Equatable, Sendable {
    ...
    public init(isLoading: Bool = true, isRefreshing: Bool = false, hasLoadedData: Bool = false, updates: [SoftwareUpdateItem] = [], stats: UpdatesStats = .empty, monthlyData: [MonthlyUpdateCount] = [], longestInstalledId: Int? = nil, filterMonths: Int? = nil, errorMessage: String? = nil) {
...
public struct SoftwareUpdatesPageSnapshot: Equatable, Sendable {
    public var state: SoftwareUpdatesState
    public var allUpdates: [UpdateData]

    public init(state: SoftwareUpdatesState, allUpdates: [UpdateData]) {
        self.state = state
        self.allUpdates = allUpdates
    }
}

@MainActor
public final class SoftwareUpdatesViewModel: ObservableObject {
    private static let sharedStateCache = VehiclePageStateCache<SoftwareUpdatesPageSnapshot>(
        maximumEntryCount: 4
    )
    ...
    private let snapshotStore: any SoftwareUpdateSnapshotStoring
    private let cacheKey: VehiclePageCacheKey?
    private let stateCache: VehiclePageStateCache<SoftwareUpdatesPageSnapshot>
    ...
    public init(
        ...
        snapshotStore: any SoftwareUpdateSnapshotStoring = EmptySoftwareUpdateSnapshotStore(),
        cacheKey: VehiclePageCacheKey? = nil,
        stateCache: VehiclePageStateCache<SoftwareUpdatesPageSnapshot>? = nil,
        initialState: SoftwareUpdatesState = SoftwareUpdatesState()
    ) {
        ...
        self.allUpdates = restored?.allUpdates ?? []
    }
    ...
    public func load(carId: Int) async {
        ...
        let serverURL = await settingsStore?.load().serverURL ?? ""
        if !state.hasLoadedData,
           !serverURL.isEmpty,
           let snapshot = await snapshotStore.load(serverURL: serverURL, carId: carId) {
            allUpdates = snapshot.updates
            state.hasLoadedData = true
            applyFilter()
            state.isLoading = false
            saveCachedState()
        }
        switch await api.updates(carId: carId, page: 1, show: 200) {
        case let .success(updates):
            ...
            if !serverURL.isEmpty {
                await snapshotStore.save(
                    SoftwareUpdateSnapshot(updates: updates),
                    serverURL: serverURL,
                    carId: carId
                )
            }
    ...
    private func saveCachedState() {
        ...
        snapshotState.errorMessage = nil
        stateCache.save(
            SoftwareUpdatesPageSnapshot(state: snapshotState, allUpdates: allUpdates),
            for: cacheKey
        )
    }
