...
public protocol PlaceActivitySummaryProviding: Sendable {
    func activities(carId: Int) async throws -> [TeslaMateActivity]
}

public struct DatabasePlaceActivitySummaryProvider: PlaceActivitySummaryProviding {
    ...
    public func activities(carId: Int) async throws -> [TeslaMateActivity] {
        ...
        let driveRecords = try await drives
        let chargeRecords = try await charges
        return driveRecords.map(Self.activity) + chargeRecords.compactMap(Self.activity)
    }

    private static func activity(_ record: DriveSummaryRecord) -> TeslaMateActivity {
        TeslaMateActivity(
            id: record.driveId,
            type: "drive",
            startDate: record.startDate,
            endDate: record.endDate,
            durationMin: record.durationMin.map(Double.init),
            ...
            endLatitude: nil,
            endLongitude: nil,
            kwh: record.energyConsumedNet.map { -abs($0) },
            kwhUsed: nil,
            cost: nil,
            rangeDiffKm: nil,
            soc: record.endBatteryLevel,
            socDiff: batteryDifference(record.startBatteryLevel, record.endBatteryLevel),
            odometerKm: nil,
            distanceKm: record.distance,
            endRangeKm: record.endRatedRangeKm
        )
    }

    private static func activity(_ record: ChargeSummaryRecord) -> TeslaMateActivity? {
        guard record.endDate != nil else { return nil }
        return TeslaMateActivity(
            id: record.chargeId,
            type: "charge",
            startDate: record.startDate,
            endDate: record.endDate,
            ...
            endAddress: nil,
            startLatitude: record.latitude,
            startLongitude: record.longitude,
            endLatitude: nil,
            endLongitude: nil,
            kwh: record.chargeEnergyAdded,
            kwhUsed: record.chargeEnergyUsed,
            cost: record.cost,
            rangeDiffKm: nil,
            soc: record.endBatteryLevel,
            socDiff: batteryDifference(record.startBatteryLevel, record.endBatteryLevel),
...
public final class PlaceInsightsViewModel: ObservableObject {
    private static let sharedStateCache = VehiclePageStateCache<PlaceInsightsState>()
    ...
    private let summaryProvider: (any PlaceActivitySummaryProviding)?
    private let cacheKey: VehiclePageCacheKey?
    private let stateCache: VehiclePageStateCache<PlaceInsightsState>
    private var isRefreshing = false

    public init(
        api: any ActivityAPIProviding,
        placesAPI: (any ServerPlacesAPIProviding)? = nil,
        settingsStore: (any SettingsStoring)? = nil,
        summaryProvider: (any PlaceActivitySummaryProviding)? = nil,
        cacheKey: VehiclePageCacheKey? = nil,
        stateCache: VehiclePageStateCache<PlaceInsightsState>? = nil,
        initialState: PlaceInsightsState = PlaceInsightsState()
    ) {
    ...
    public func load(carId: Int) async {
        ...
        state.currencyCode = settings?.resolvedCurrencyCode() ?? MateDriveCurrencyFormatter.systemCurrencyCode()
        var hasLocalSummary = !state.places.isEmpty
        if let summaryProvider,
           let activities = try? await summaryProvider.activities(carId: carId),
           !activities.isEmpty {
            let parkingFeeRules = settings?.parkingFeeRules ?? []
            state.places = await Task.detached(priority: .userInitiated) {
                Self.aggregate(activities, parkingFeeRules: parkingFeeRules)
            }.value
            state.activityCount = activities.count
        ...
        if let placesAPI, let serverResult = await loadServerPlaces(carId: carId, api: placesAPI), !serverResult.places.isEmpty {
            state.places = await Task.detached(priority: .userInitiated) {
                serverResult.places
                    .map(Self.mapServerPlace)
                    .sorted { ($0.visitCount ?? 0) > ($1.visitCount ?? 0) }
            }.value
        ...
        if let error = activities.state.errorMessage, activities.state.items.isEmpty, !hasLocalSummary {
            state.errorMessage = error
        } else if !activities.state.items.isEmpty {
            let activityItems = activities.state.items
            let parkingFeeRules = settings?.parkingFeeRules ?? []
            state.places = await Task.detached(priority: .userInitiated) {
                Self.aggregate(activityItems, parkingFeeRules: parkingFeeRules)
            }.value
            state.usesServerPlaces = false
            state.serverRecordCount = nil
            state.serverUnits = nil
        }
        if !activities.state.items.isEmpty {
