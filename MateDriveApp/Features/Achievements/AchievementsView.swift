...
public struct AchievementsView: View {
    ...
    @State private var sharedAchievement: TeslaMateAchievement?
    ...
    public var body: some View {
        Group {
            if viewModel.state.isLoading, !viewModel.state.hasLoadedData {
                ...
                LoadingStateView(
                    title: t("Achievements unavailable", "成就不可用"),
                ...
                ScrollView {
                    LazyVStack(spacing: 12) {
                        ...
                        summary
                        if viewModel.state.achievements.isEmpty {
                            LoadingStateView(
                                title: t("No achievements available", "暂无成就数据"),
                                systemImage: "medal"
                            )
                        } else {
        ...
        .accessibilityIdentifier("achievements_view")
        .sheet(item: $sharedAchievement) { achievement in
            AchievementShareComposerView(
        ...
        return VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top, spacing: 12) {
                ...
                HStack(spacing: 8) {
                    ...
                    Button {
                        sharedAchievement = achievement
                    } label: {
                        Image(systemName: "square.and.arrow.up")
                            .frame(width: 30, height: 30)
                    }
                    .buttonStyle(.plain)
                    .accessibilityLabel(t("Share Achievement", "分享成就"))
                }
            }
            if let bestTier {
    ...
    private func value(_ value: Double?, unit: String?) -> String {
        ...
        switch unit {
        case "km": return MateDriveUnitFormatter.formatDistance(value, units: units, decimals: 1)
        case "m": return MateDriveUnitFormatter.formatElevation(Int(value.rounded()), units: units)
        case "°C": return MateDriveUnitFormatter.formatTemperature(value, units: units, decimals: 1)
        case "days": return t("\(format(value)) days", "\(format(value)) 天")
        case "min": return MateDriveUnitFormatter.formatDuration(minutes: Int(value.rounded()), language: appLanguage)
