...
public struct RegionsVisitedState: Equatable, Sendable {
    ...
    public init(isLoading: Bool = true, regions: [RegionVisitRecord] = [], sortOrder: RegionSortOrder = .firstVisit, units: UnitPreferences? = nil, errorMessage: String? = nil, isEnrichingLocations: Bool = false, enrichmentResult: GeographyEnrichmentResult? = nil) {
...
public final class RegionsVisitedViewModel: ObservableObject {
    ...
    public init(api: any AnalyticsAPIProviding, geographyEnricher: (any HistoricalGeographyEnriching)? = nil, initialState: RegionsVisitedState = RegionsVisitedState()) {
        self.api = api
    ...
    public func load(carId: Int, countryName: String, year: Int?) async {
        ...
        case let (.success(drives), .success(charges)):
            ...
            originalRegions = Self.aggregate(drives: drives, charges: charges, countryName: countryName, year: year)
            applySort()
