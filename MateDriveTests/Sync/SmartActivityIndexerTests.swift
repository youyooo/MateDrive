...
final class SmartActivityIndexerTests: XCTestCase {
    ...
    func testForceRebuildReplacesOnlySuccessfulCarsAndPreservesFailedCarData() async throws {
        let priorFailedCar = Self.previousSession(carId: 2)
        let store = InMemorySmartActivitySessionStore(initial: [2: [priorFailedCar]])
        let source = MutableSmartActivitySourceLoader(
            snapshots: [1: Self.snapshot(carId: 1), 2: Self.snapshot(carId: 2)],
            failedCarIds: [2]
        )
        let indexer = SmartActivityIndexer(
            source: source,
        ...
        _ = await indexer.rebuild(carIds: [1])
        let replaceCountBeforeForce = await store.replaceCount
        let report = await indexer.forceRebuild(carIds: [2, 1])
        let replaceCountAfterForce = await store.replaceCount
        let failedCarSessions = try await store.sessions(carId: 2)

        XCTAssertEqual(report.completedCarIds, [1])
        XCTAssertEqual(report.failedCarIds, [2])
        XCTAssertEqual(replaceCountAfterForce, replaceCountBeforeForce + 1)
        XCTAssertEqual(failedCarSessions, [priorFailedCar])
    }
