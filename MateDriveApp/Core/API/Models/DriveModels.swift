...
public struct DriveRange: Decodable, Equatable, Sendable {
    ...
    public var usedDistance: Double? {
        if let startRange, let endRange,
           startRange.isFinite, endRange.isFinite {
            return max(startRange - endRange, 0)
        }
        guard let rangeDiff, rangeDiff.isFinite else { return nil }
        return abs(rangeDiff)
    }
}
