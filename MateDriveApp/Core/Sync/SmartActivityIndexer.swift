...
public extension SmartActivityIndexing {
    func knownCarIds() async throws -> [Int] { [] }

    func forceRebuild(carIds: [Int]) async -> SmartActivityIndexReport {
        await rebuild(carIds: carIds)
    }
}
...
public actor SmartActivityIndexer: SmartActivityIndexing {
    ...
    public func knownCarIds() async throws -> [Int] {
        try await sessionStore.knownCarIds()
    }

    public func rebuild(carIds: [Int]) async -> SmartActivityIndexReport {
        await performRebuild(carIds: carIds, force: false)
    }

    public func forceRebuild(carIds: [Int]) async -> SmartActivityIndexReport {
        await performRebuild(carIds: carIds, force: true)
    }

    private func performRebuild(carIds: [Int], force: Bool) async -> SmartActivityIndexReport {
        ...
        for (index, carId) in attempted.enumerated() {
            ...
            do {
                ...
                if !force, Self.matchesFingerprint(
                    existing,
    ...
    nonisolated private static func resolvedChargeCost(
        ...
        let sessionGeofenceKind = session.geofenceID.flatMap { id in
        ...
        let resolutions = charges.compactMap { reference -> ResolvedCharge? in
               ...
               measuredIdentity != .ac
            {
                ...
                chargerIdentity = ChargingSessionAnalyzer.chargerIdentity(
                    isDc: true,
            ...
            let billing = ChargeBillingEvidence.energyAndSamples(
                wallEnergy: activity.kwhUsed,
                batteryEnergy: activity.kwh,
                samples: aggregate?.energySamples ?? []
            )
            ...
            let inferenceRecord = ChargeSummaryRecord(
                ...
                startDate: activity.startDate ?? "",
                endDate: activity.endDate,
                chargeEnergyAdded: activity.kwh,
                cost: activity.cost,
                durationMin: activity.durationMin.flatMap {
                    $0.isFinite && $0 >= 0 ? Int($0.rounded()) : nil
                },
                address: activity.startAddress ?? activity.endAddress,
                latitude: activity.startLatitude ?? activity.endLatitude,
                longitude: activity.startLongitude ?? activity.endLongitude,
                chargeEnergyUsed: activity.kwhUsed,
                odometerKm: activity.odometerKm
            )
            let inferredGeofenceKind = sessionGeofenceKind
                ?? resolvedIsDc.flatMap {
            ...
            guard let resolution = ChargeCostResolver.resolve(ChargeCostResolutionInput(
                ...
                geofenceKind: inferredGeofenceKind,
    ...
    nonisolated private static func regionalPricingRule(
        ...
        guard let regionCode = settings.residentialTariffRegionCode
                ?? catalog.regionCode(matchingAddress: address),
              let date = startDate.flatMap(DomainDateParser.date(from:)),
              let entry = catalog.entryForEstimate(regionCode: regionCode, date: date)
        else { return nil }
        var rule = catalog.makePricingRule(regionCode: regionCode, from: entry)
        if entry.effectiveToDate != nil, catalog.entry(regionCode: regionCode, date: date) == nil {
            rule.id += "-reference-estimate"
            rule.name += " reference estimate"
