...
public struct StandbyDrainView: View {
    ...
    private func scopeNotice(_ data: StandbyDrainData) -> some View {
        ...
        let radius = data.radiusMeters.map { MateDroidUnitFormatter.formatDistance($0 / 1_000, units: effectiveUnits, decimals: 1) } ?? "--"
    ...
    private func distance(_ value: Double?) -> String {
        ...
        return MateDroidUnitFormatter.formatDistance(value, units: effectiveUnits, decimals: 1)
    }

    private func rate(_ value: Double?) -> String {
        guard let value else { return "--" }
        let converted = MateDroidUnitFormatter.distanceValue(value, units: effectiveUnits)
        return String(format: "%.2f %@/h", converted, MateDroidUnitFormatter.distanceUnit(units: effectiveUnits))
    }
