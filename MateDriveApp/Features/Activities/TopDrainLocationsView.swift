...
public struct TopDrainLocationsView: View {
    ...
    private let standbyDrainCacheKey: (Double, Double) -> VehiclePageCacheKey?

    public init(
        carId: Int,
        viewModel: TopDrainLocationsViewModel,
        standbyDrainAPI: any ActivityAPIProviding,
        standbyDrainCacheKey: @escaping (Double, Double) -> VehiclePageCacheKey? = { _, _ in nil }
    ) {
        self.carId = carId
        self.standbyDrainAPI = standbyDrainAPI
        self.standbyDrainCacheKey = standbyDrainCacheKey
    ...
    public var body: some View {
        Group {
            if viewModel.state.isLoading, !viewModel.state.hasLoadedData {
                ...
                LoadingStateView(
                    title: t("Standby hotspots unavailable", "待机耗电热点不可用"),
                    message: error,
        ...
        .accessibilityIdentifier("top_drain_locations_view")
        .toolbar {
    ...
    private func detail(_ location: TopDrainLocation) -> some View {
                ...
                if let coordinate = coordinate(location) {
                    ...
                    NavigationLink {
                        StandbyDrainView(
                            ...
                            placeName: locationName(location, index: nil),
                            preferredUnits: effectiveUnits,
                            viewModel: StandbyDrainViewModel(
                                api: standbyDrainAPI,
                                cacheKey: standbyDrainCacheKey(
                                    coordinate.latitude,
        ...
        .accessibilityIdentifier("standby_hotspot_detail")
    }
    ...
    private func distance(_ value: Double?) -> String { value.map { MateDriveUnitFormatter.formatDistance($0, units: effectiveUnits, decimals: 1) } ?? "--" }
    private func rate(_ value: Double?) -> String { guard let value else { return "--" }; return String(format: "%.2f %@/h", MateDriveUnitFormatter.distanceValue(value, units: effectiveUnits), MateDriveUnitFormatter.distanceUnit(units: effectiveUnits)) }
    private func percent(_ value: Double?) -> String { value.map { String(format: "%+.1f%%/24h", $0) } ?? "--" }
    private func duration(_ value: Double?) -> String { value.map { MateDriveUnitFormatter.formatDuration(minutes: Int($0.rounded()), language: appLanguage) } ?? "--" }
