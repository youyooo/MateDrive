...
public struct TeslaMateConnectionDiagnostic: TeslaMateConnectionDiagnosing {
    ...
    public func run(settings: AppSettings, authenticator: RequestAuthenticator, language: AppLanguage) async -> TeslaMateDiagnosticReport {
        ...
        for serverURL in serverURLs where seenURLs.insert(serverURL).inserted {
            ...
            switch TeslaMateServerURLPolicy.evaluate(serverURL) {
            case .insecureRemoteHTTP:
                let report = TeslaMateDiagnosticReport(checks: [
                    .failed(
                        ...
                        "Server Security",
                        "服务器安全",
                        "Remote TeslaMate servers must use HTTPS. Plain HTTP is allowed only for local network addresses.",
                        "远程 TeslaMate 服务器必须使用 HTTPS；只有本地网络地址可以使用明文 HTTP。",
                        language: language
                    )
                ])
                lastReport = report
            ...
            case .invalid:
                let report = TeslaMateDiagnosticReport(checks: [
                    .failed(
                        ...
                        "Server",
                        "服务器",
                        "The TeslaMate server URL must use HTTP or HTTPS.",
                        "TeslaMate 服务器地址必须使用 HTTP 或 HTTPS。",
                        language: language
                    )
                ])
                lastReport = report
                continue
            case .secureRemote, .localHTTP:
                break
            }
            ...
            let acceptsInvalidCertificates = settings.acceptInvalidCerts
                && TeslaMateServerURLPolicy.permitsInvalidCertificateBypass(serverURL)
            let client = clientOverride
                ?? URLSessionHTTPClient(acceptsInvalidCertificates: acceptsInvalidCertificates)
            ...
            case let .success(value):
                let candidateCar = settings.lastSelectedCarId.flatMap { selectedID in
                    value.first(where: { $0.carId == selectedID })
                } ?? value[0]
                switch await api.carStatus(carId: candidateCar.carId) {
                case let .success(payload):
                    selectedCars = value
                    selectedAPI = api
                    checks = candidateChecks
                    checks.append(.passed(.cars, "Vehicles", "车辆", "Found \(value.count) vehicle(s).", "找到 \(value.count) 辆车。", language: language))
                    checks.append(vehicleInfoCheck(for: candidateCar, language: language))
                    checks.append(statusCheck(for: payload, language: language))
                case let .failure(error):
                    candidateChecks.append(.passed(.cars, "Vehicles", "车辆", "Found \(value.count) vehicle(s).", "找到 \(value.count) 辆车。", language: language))
                    candidateChecks.append(vehicleInfoCheck(for: candidateCar, language: language))
                    candidateChecks.append(.failed(.status, "Status", "状态", "Vehicle status failed: \(message(for: error, language: .english))", "车辆状态失败：\(message(for: error, language: .chinese))", language: language))
            ...
            case let .failure(error):
                candidateChecks.append(.failed(
                    .cars,
                    "Vehicles",
                    "车辆",
                    "Vehicle list failed: \(connectionFailureMessage(for: error, serverURL: serverURL, language: .english))",
                    "车辆列表失败：\(connectionFailureMessage(for: error, serverURL: serverURL, language: .chinese))",
                    language: language
                ))
                let report = TeslaMateDiagnosticReport(checks: candidateChecks)
                lastReport = report
    ...
    private func vehicleInfoCheck(for car: CarData, language: AppLanguage) -> TeslaMateDiagnosticCheck {
        ...
        if !hasModel && !hasUsableName {
            return .warning(
                ...
                "Vehicle info",
                "车辆信息",
                "Vehicle list is readable, but model and vehicle name are missing. Vehicle identification is incomplete.",
                "可以读取车辆列表，但车型和车辆名称缺失，车辆识别信息不完整。",
                language: language
            )
        }
        ...
        if !hasModel {
            return .warning(
                ...
                "Vehicle info",
                "车辆信息",
                "Vehicle name is readable, but model is missing. Vehicle identification is incomplete.",
                "可以读取车辆名称，但车型缺失，车辆识别信息不完整。",
                language: language
            )
        }
        ...
        if !hasExteriorColor || !hasWheelType {
            ...
            return .warning(
                ...
                "Vehicle info",
                "车辆信息",
                "Vehicle model \(englishModel) is readable, but exterior color or wheel type is missing.",
                "可以识别车型 \(chineseModel)，但外观颜色或轮毂类型缺失。",
                language: language
            )
        }
    ...
    private func historyDataQualityCheck(api: TeslamateAPI, carId: Int, language: AppLanguage) async -> TeslaMateDiagnosticCheck {
        async let drivesResult = VehicleHistoryPaginator.drives(loadPage: { page, show in
            await api.drives(carId: carId, page: page, show: show)
        })
        async let chargesResult = VehicleHistoryPaginator.charges(loadPage: { page, show in
            await api.charges(carId: carId, page: page, show: show)
        })
    ...
    private func message(for error: APIError, language: AppLanguage) -> String {
        ...
        case .cancelled:
            return localized("Request cancelled.", "请求已取消。", language: language)
        case .emptyBody:
            return localized("Empty response.", "响应为空。", language: language)
        }
    }

    private func connectionFailureMessage(for error: APIError, serverURL: String, language: AppLanguage) -> String {
        guard case .network = error else {
            return message(for: error, language: language)
        }

        if TeslaMateServerURLPolicy.isTailscaleHost(serverURL) {
            return localized(
                "Tailscale host is unreachable. Confirm the Mac mini is online and signed in to Tailscale. If the iPhone is on the same LAN, switch to the Mac's stable .local hostname or another reachable HTTPS endpoint.",
                "Tailscale 主机当前不可达。请确认 Mac mini 在线并已登录 Tailscale；如果 iPhone 与 Mac 在同一局域网，可切换到 Mac 的稳定 .local 主机名或其他可访问的 HTTPS 地址。",
                language: language
            )
        }

        if TeslaMateServerURLPolicy.isLocalNetworkHost(serverURL) {
            return localized(
                "Local TeslaMate host is unreachable. Its address may have changed; use a stable .local hostname or a DHCP reservation.",
                "局域网 TeslaMate 主机当前不可达，可能是地址发生了变化；请改用稳定的 .local 主机名或 DHCP 保留地址。",
                language: language
            )
        }
