...
public protocol ChargeAPIProviding: ChargeCostUpdating {
    ...
    func refreshCurrentCharge(carId: Int) async -> APIResult<CurrentChargeOutcome>
    ...
    func refreshCarStatus(carId: Int) async -> APIResult<CarStatusPayload>
    func updateChargeCost(chargeId: Int, cost: Double?) async -> APIResult<Void>
}

public extension ChargeAPIProviding {
    func refreshCurrentCharge(carId: Int) async -> APIResult<CurrentChargeOutcome> {
        await currentCharge(carId: carId)
    }

    func refreshCarStatus(carId: Int) async -> APIResult<CarStatusPayload> {
        await carStatus(carId: carId)
    }
...
public struct SettingsBackedChargeAPI: ChargeAPIProviding {
    ...
    public func refreshCurrentCharge(carId: Int) async -> APIResult<CurrentChargeOutcome> {
        await apiFactory.request { api in
            await api.refreshCurrentCharge(carId: carId)
        }
    }
    ...
    public func refreshCarStatus(carId: Int) async -> APIResult<CarStatusPayload> {
        await apiFactory.request { api in
            await api.refreshCarStatus(carId: carId)
        }
    }
