...
final class TripRuleTests: XCTestCase {
    func testSummaryPreservesKnownAndMissingTotals() throws {
        let drives = [
        ...
        let trip = try XCTUnwrap(JourneySummaryBuilder.makeSummary(drives: drives, charges: charges, name: "Road Trip"))
        ...
        XCTAssertEqual(trip.drivingEnergyKnownCount, 1)
        XCTAssertEqual(trip.missingDrivingEnergyCount, 1)
        XCTAssertFalse(trip.drivingEnergyIsComplete)
        XCTAssertEqual(trip.totalEnergyCharged, 65)
        XCTAssertEqual(trip.chargedEnergyKnownCount, 2)
        XCTAssertEqual(trip.missingChargedEnergyCount, 0)
        XCTAssertTrue(trip.chargedEnergyIsComplete)
        ...
        XCTAssertNil(trip.averageEfficiency)
    ...
    func testAggregatorPreservesMissingEnergyAndMaximumSpeed() throws {
        let trip = try XCTUnwrap(JourneySummaryBuilder.makeSummary(
            drives: [
                drive(1, "2026-01-01T08:00:00Z", "2026-01-01T09:00:00Z", distance: 50, duration: 60, energy: nil, speed: nil)
            ],
            charges: [
                charge(1, "2026-01-01T09:05:00Z", "2026-01-01T09:35:00Z", energy: nil)
            ]
        ))

        XCTAssertNil(trip.totalEnergyConsumed)
        XCTAssertEqual(trip.drivingEnergyKnownCount, 0)
        XCTAssertEqual(trip.missingDrivingEnergyCount, 1)
        XCTAssertFalse(trip.drivingEnergyIsComplete)
        XCTAssertNil(trip.averageEfficiency)
        XCTAssertNil(trip.totalEnergyCharged)
        XCTAssertEqual(trip.chargedEnergyKnownCount, 0)
        XCTAssertEqual(trip.missingChargedEnergyCount, 1)
        XCTAssertFalse(trip.chargedEnergyIsComplete)
        XCTAssertNil(trip.maxSpeed)
    }

    func testAggregatorKeepsKnownZeroValuesDistinctFromMissingValues() throws {
        let trip = try XCTUnwrap(JourneySummaryBuilder.makeSummary(
            drives: [
                drive(1, "2026-01-01T08:00:00Z", "2026-01-01T09:00:00Z", distance: 50, duration: 60, energy: 0, speed: 0)
            ],
            charges: [
                charge(1, "2026-01-01T09:05:00Z", "2026-01-01T09:35:00Z", energy: 0)
            ]
        ))

        XCTAssertEqual(trip.totalEnergyConsumed, 0)
        XCTAssertTrue(trip.drivingEnergyIsComplete)
        XCTAssertEqual(trip.averageEfficiency, 0)
        XCTAssertEqual(trip.totalEnergyCharged, 0)
        XCTAssertTrue(trip.chargedEnergyIsComplete)
